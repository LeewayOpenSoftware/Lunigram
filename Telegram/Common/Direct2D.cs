        ...
        public static Direct2DDevice Current
        {
            get
            {
                if (_current == null)
                {
                    ...
                    {
#if LINUX
                        // The managed stand-in draws through the window, not a compositor.
                        _current = new Direct2DDevice(WindowContext.Current);
#else
            ...
            get
            {
                lock (_sharedLock)
                {
                    if (_shared == null)
                    {
                        ...
                        {
#if LINUX
                            _shared = new Direct2DDevice(null as Window);
#else
