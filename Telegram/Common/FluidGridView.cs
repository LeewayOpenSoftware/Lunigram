...
namespace Telegram.Common
{
    public partial class FluidGridView
    {
        ...
        // u-stickers (2026-09-05): this setter is NEVER CALLED, and it has to exist anyway.
        //
        // Uno's XAML generator will only resolve `<common:FluidGridView.Triggers>` as a property
        // element if the attached property has BOTH accessors. With a getter alone it silently
        // gives up on the whole enclosing scope: every sibling AFTER the control carrying the
        // Triggers element stops being emitted, with 0 errors and 0 warnings. That is what hid
        // StickersPopup's MoreButton and ProfileGiftsTabPage's empty state.
        //
        // The generated code still goes through GetTriggers -- it emits
        // `FluidGridView.GetTriggers(x).Add(...)`, so the lazy wiring above (CollectionChanged,
        // SizeChanged) is preserved and this setter never runs. It exists purely so the member
        // resolves. Do not "clean it up" as dead code.
        public static void SetTriggers(DependencyObject obj, FluidGridViewTriggerCollection value)
        {
    ...
    public abstract partial class FluidGridViewTriggerBase : DependencyObject
    {
        #region MinLength
