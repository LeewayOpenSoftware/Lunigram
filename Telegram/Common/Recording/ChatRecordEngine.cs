...
namespace Telegram.Common.Recording
{
    ...
    // The engine itself is MediaCapture + MediaFrameReader + VoiceSink from top to bottom, none
    // of which Uno/Skia implements. The Linux twin -- same events, same queue, same too-short rule
    // -- is Telegram.Linux/Platform/ChatRecordEngine.cs, on PulseAudio capture and the Opus
    // encoder of libunigram-native. ChatRecordMode and ChatRecordResult above are shared: they are
    // what ChatRecordSession, ChatRecordButton and ChatRecordBar speak in.
#if !LINUX
