...
namespace Telegram.Common.Recording
{
    ...
    /// </summary>
    /// <remarks>
    /// Windows only, and not because of the encoder: what is Windows-only is the AudioFrame it
    /// unpacks. The Linux recorder (Telegram.Linux/Platform/ChatRecordEngine.cs) hands the capture
    /// buffer straight to OpusOutput.Write(ReadOnlySpan&lt;float&gt;), so there is no frame to
    /// unpack and no 20 ms buffer to keep -- the encoder buffers the remainder itself.
    /// </remarks>
