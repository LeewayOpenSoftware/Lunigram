...
namespace Telegram.Common
{
    ...
    public partial class AnimatedListHandler
    {
        ...
        public void UpdateVisibleItems(bool load)
        {
...
#if LINUX
            // ItemsStackPanel and ItemsWrapGrid have FOUR index getters -- FirstVisibleIndex,
            // LastVisibleIndex, FirstCacheIndex, LastCacheIndex -- and on Uno every one of them is
            // NotImplemented and THROWS (PORTING.md 6 measured the IL: ldstr; newobj
            // NotImplementedException; throw). This method is the whole of LoadVisibleItems,
            // UnloadVisibleItems and ThrottleVisibleItems, which every drawer and StickerPanel call
            // on activation and on each tab switch, so reading them here does not degrade the
            // animation bookkeeping -- it throws out of the first tab switch.
            //
            // Same substitute Extensions.ForEach already uses for the same two panels: what the
            // index range is FOR is "walk the containers that exist right now", and the panel holds
            // exactly those in Children -- a virtualizing panel adds and removes them as it
            // materializes and recycles. Visible vs merely cached is then decided by geometry
            // against the scroll viewport, which is measurable, instead of by an index that throws.
            var panel = _listView.ItemsPanelRoot;
            if (panel == null)
            {
                return;
            ...
            var viewport = _listView.GetScrollViewer();
            var children = panel.Children;

            for (int i = 0; i < children.Count; i++)
            {
                if (children[i] is not SelectorItem container || container.ContentRoot() is not FrameworkElement content)
                {
                    continue;
                }
                ...
                player?.ViewportChanged(load && IsWithinViewport(container, viewport));
            }

            _unloaded = !load;
        }

        // True when the container overlaps the scroller's visible rectangle. Deliberately
        // permissive: with no viewport to compare against, everything materialized counts as
        // visible, which is what the panel's cache range would have said anyway.
        private static bool IsWithinViewport(FrameworkElement container, ScrollViewer viewport)
        {
            if (viewport == null)
            {
                return true;
            ...
            {
                // TransformToVisual throws when the element is not in the same tree yet.
                return false;
            }
        }

        private void UpdateVisibleItemsWindows(bool load)
        {
#endif
            ...
            for (int i = start; descending ? i >= stop : i <= stop; i += step)
            {
                ...
                if (container == null || container.ContentRoot() is not FrameworkElement content)
                {
                    continue;
                }
...
#if LINUX
        // Kept, not deleted: it is the Windows body of UpdateVisibleItems and the reference for
        // what the Linux one above has to reproduce. Never called on this head.
#endif
    }
}
