...
#if LINUX
using ApplicationData = Telegram.Services.LocalApplicationData;
...
namespace Telegram.Common
{
    public partial class Theme : ResourceDictionary
    {
        ...
        public void UpdateEmojiSet()
        {
...
#if LINUX
            // Uno Skia reads FontFamily.Source as a single family name: FontDetailsCache truncates
            // it at the first '#' - so the "#Segoe UI Emoji" alias never registers a name - and it
            // never splits a comma separated list, so every string built above matches no font at
            // all and the run ends up on whatever fontconfig returns for it, keeping the metrics of
            // a face that has none of its glyphs. One family per key here instead; the codepoints
            // that family is missing come back through LinuxFontFallback, which Program.Main
            // registers as Uno's per-codepoint fallback service.
            // UNIGRAM_PLAIN_FONTS=1 also keeps the packaged emoji font out of that fallback.
            var plainFonts = System.Environment.GetEnvironmentVariable("UNIGRAM_PLAIN_FONTS") == "1";

            XamlAutoFontFamily = xamlAutoFontFamilyValue;
            LinuxFontFallback.UsePackagedEmoji = !plainFonts
                && AppSettings.Appearance.EmojiSet != "microsoft";

            this["EmojiTextThemeFontFamily"] = new FontFamily(XamlAutoFontFamily);
            ...
            // Telegram.ttf cannot be the family of these two without losing the text they mostly
            // carry, so its glyphs also arrive through LinuxFontFallback - except for the 335
            // codepoints Uno's Fluent symbols font defines as well and answers for first. Nunito is
            // dropped: it is a variable font, which Skia only resolves through a .ttf.manifest
            // pointing at static instances, and there are none packaged.
            this["EmojiThemeFontFamilyWithSymbols"] = new FontFamily(XamlAutoFontFamily);
            this["EmojiThemeFontFamilyWithRounded"] = new FontFamily(XamlAutoFontFamily);

            // Generic fontconfig aliases: neither "Times New Roman" nor "Cascadia Mono" is
            // installed here, and a list would not be honoured anyway.
            this["EmojiThemeFontFamilyWithSerif"] = new FontFamily("serif");
            _monospaceFontFamily = new FontFamily("monospace");
#else
