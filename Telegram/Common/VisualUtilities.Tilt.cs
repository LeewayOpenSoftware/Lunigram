        ...
        private sealed class PointerTilt
        {
...
#if LINUX
            // Uno's Visual.SetAnimatableProperty handles RotationAngle (radians) and not
            // RotationAngleInDegrees, and an animation on a property it does not handle throws
            // once per rendered frame from inside Compositor.RenderRootVisual for as long as the
            // animation lives. Same rotation, expressed in radians. The plain property
            // assignments below (RotationAngleInDegrees = 0) are not animations and stay as they
            // are.
            private const string RotationProperty = "RotationAngle";
            private const float RotationUnit = MathF.PI / 180;
#else
            private const string RotationProperty = "RotationAngleInDegrees";
            private const float RotationUnit = 1;
#endif
            ...
            public void Detach()
            {
                ...
                if (_visual != null)
                {
                    ...
                    _visual.StopAnimation(RotationProperty);
                ...
                {
                    _sheenVisual.StopAnimation(RotationProperty);
            ...
            private void Initialize()
            {
                ...
                angle.SetScalarParameter("degrees", _degrees * RotationUnit);
                _visual.StartAnimation(RotationProperty, angle);
                ...
                if (_sheen != null)
                {
                    ...
                    sweep.SetScalarParameter("degrees", _sheenDegrees * RotationUnit);
                    _sheenVisual.StartAnimation(RotationProperty, sweep);
...
#if LINUX
                    // See Telegram.Linux/Xaml/CompositionRenderingClock.cs: Uno's
                    // CompositionTarget.Rendering is not a frame clock.
