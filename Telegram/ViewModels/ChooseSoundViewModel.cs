...
#if LINUX
        // Uploading a CUSTOM notification sound is the one thing on this popup that does not work
        // here, and it is not the picker's fault: Extensions.ToGeneratedAsync - which turns a
        // StorageFile into the InputFileGenerated that AddSavedNotificationSound needs - lives
        // inside the #if !LINUX block of Common/Extensions.cs (:1423 onwards), because the
        // generation service it depends on (Services/GenerationService.cs) is not in the subset
        // either. So the row is hidden from the popup rather than left as a link that opens a file
        // dialog and then silently drops the file. Everything else on this screen works: the list
        // of the account's saved sounds is real (getSavedNotificationSounds), and picking one of
        // them, or "None", or the default, goes to TDLib as usual.
        public void Upload()
        {
            Logger.Info("Uploading a notification sound needs the generation service, which is not in the Linux subset");
        }
#else
