...
namespace Telegram.ViewModels
{
    public partial class DialogViewModel
    {
        ...
        // u-073 / PARIDAD M25b. Was #if !LINUX entirely because StickersPopup was outside the
        // subset. It has been in since u-052, generator-truncation bug and all (see the #if LINUX
        // block in StickersPopup.xaml.cs's own constructor -- BuildMoreButton) -- this was the one
        // remaining caller still guarded.
...
#else
            // Historias (StoriesWindow) e InstantGalleryViewModel (el álbum de un Instant View) siguen
            // fuera del subconjunto; el resto es el camino real de la galería, ya portado.
            else if (message.Content is MessageAnimation || (message.Content is MessageText text2 && text2.LinkPreview?.Type is LinkPreviewTypeAnimation))
            {
                Delegate?.PlayMessage(message, target);
            }
            else
        ...
        private async void UpdateSelectionState()
        {
...
#if LINUX
            // 2026-08-27, segunda pasada. Estas dos propiedades gobiernan el IsEnabled de los
            // botones «Reenviar» y «Eliminar» de la barra de seleccion multiple (ChatView.xaml), a
            // la que se llega arrastrando sobre las burbujas sin pasar por ningun menu. §1.13 las
            // puso las dos a false porque los dos metodos estaban vacios y los botones salian
            // HABILITADOS y mudos.
            //
            // ELIMINAR YA NO ESTA VACIO: DialogViewModel.Linux.cs implementa
            // DeleteSelectedMessages con una confirmacion propia sobre MessagePopup. Asi que esta
            // propiedad vuelve a calcularse de verdad, exactamente como en Windows, y el boton de
            // la papelera vuelve a habilitarse -- pero ahora borra.
            //
            // REENVIAR YA NO ESTA VACIO tampoco (2026-09-02, PARIDAD §1.21; CC-3 2026-09-04 lo paso
            // al selector entero): DialogViewModel.Forward.Linux.cs lo implementa sobre
            // ChooseChatsPopup, el mismo de upstream. Asi que esta propiedad tambien vuelve a
            // calcularse y el boton de reenviar de la barra vuelve a habilitarse -- pero ahora
            // reenvia.
            //
            // La rama WelcomeMessages tampoco entra: se borra con deleteChatWelcomeMessage y esa
            // funcion no existe en el TDLib vendorizado, asi que ahi el boton se queda gris.
            ...
            else if (Type is DialogType.WelcomeMessages)
            {
