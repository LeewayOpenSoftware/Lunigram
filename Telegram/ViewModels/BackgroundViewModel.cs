...
namespace Telegram.ViewModels
{
    ...
    public partial class BackgroundViewModel : ViewModelBase, IDelegable<IBackgroundDelegate>
    {
...
#else
        // COLLAPSED: third and last reach from this parcel into ChooseChatsPopup, same reason as the
        // other two (SettingsThemesViewModel.ShareTheme, SettingsBackgroundsViewModel.Share).
        // Its entry point is collapsed with it in BackgroundPopup.
#endif
        ...
        public async Task<BackgroundInfo> GetBackgroundAsync()
        {
            ...
            {
#if LINUX
                // COLLAPSED, and its entry point with it (the "select from gallery" button on
                // SettingsBackgroundsPage). Two reasons, both measured rather than assumed:
                //
                // 1. Getting here at all needs FileOpenPicker.PickSingleFileAsync, and FALTA 1.5 is
                //    explicit that the picker is an OPEN UNKNOWN on this head -- Uno 6.6 does ship
                //    org.freedesktop.portal.FileChooser, but the word "picker" does not appear once
                //    in HANDOFF.md and nobody has seen a file selector open on this machine. That
                //    question belongs to the attach parcel, which is where it is scoped.
                // 2. ToGeneratedAsync(ConversionType.Copy) pulls GenerationService, and behind it
                //    ImageHelper, ImageGeneration, VideoGeneration, StorageMedia/Photo/Video/Audio/
                //    Document, ImageCropper and PencilCanvas -- the media-editing stack, which is
                //    also the attach parcel's, not this one's.
                //
                // Every OTHER background path is untouched and live: the server wallpapers, the
                // patterns, and "set a colour" (ChangeToColorAsync), which needs no picker.
                return null;
#else
                try
                {
