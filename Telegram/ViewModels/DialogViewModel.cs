...
// Both configurations since parcel 3: UserEditPage (add to contacts) is in the Linux subset, and
// it is the USING that decides -- a fenced one made AddToContacts below un-writable, not just
// un-callable.
...
namespace Telegram.ViewModels
{
    ...
    public partial class DialogViewModel : ComposeViewModel, IDelegable<IDialogDelegate>
    {
...
#if LINUX
        // Plain TextBox composer on Linux: ChatTextBox is built on RichEditBox.
        public TextBox TextField { get; set; }
#else
...
#if LINUX
            field.Select(start, field.Text.Length - start);
#else
        ...
        public void SetText(string text, Vector<TextEntity> entities = null, bool focus = false)
        {
            ...
            // Uno's TextBox coerce callback for Text is
            //     if (!AcceptsReturn) { text = GetFirstLine(text); }
            //     else if (_isSkiaTextBox) { text = RemoveLF(text); }
            // (Uno.UI 6.6.184, read from the IL), so writing a multi-line value into a box that
            // does not accept returns SILENTLY KEEPS ONLY THE FIRST LINE. Measured: a two-line
            // draft saved to TDLib as "borrador final\ndos lineas" came back into the composer as
            // "borrador final". Raising it first is enough -- RemoveLF turns the '\n' into the '\r'
            // that Uno uses internally -- and ChatTextBox lowers it again as soon as the box is
            // empty, so a one-line draft still gets a one-line box.
            ...
            // Upstream leaves the cursor at the end of a restored draft; a plain TextBox leaves it
            // at 0, which puts everything typed next in front of the draft.
        ...
        public override FormattedText GetFormattedText(bool clear = false, bool parseMarkdown = true)
        {
            try
            {
...
#if LINUX
                // Plain TextBox composer: no entity tracking yet, markdown parsed by TDLib.
                ...
                // Uno's TextBox stores a line break as '\r', and TDLib THROWS LONE '\r' AWAY:
                // clean_input_string_with_entities has a literal `case '\r': // skip`
                // (Libraries/tdlib/td/telegram/MessageEntity.cpp:4217), which parseMarkdown runs.
                // Measured: a three-line box left as
                // `sendMessage { text = "borrador 03 no enviado editadolinea doslinea tres" }` --
                // every line break silently gone, the words welded together. SendMessageAsync does
                // the same replace (ComposeViewModel.cs:1134) but it runs AFTER this method has
                // already handed the text to ParseMarkdown, so it never sees a '\r'.
                text = text.Replace('\v', '\n').Replace('\r', '\n');

                return parseMarkdown
                    ? ClientEx.ParseMarkdown(text, Array.Empty<TextEntity>())
                    : new FormattedText(text, Array.Empty<TextEntity>());
#else
...
#pragma warning restore CS4014
...
#endif
            if (Type is DialogType.History && state.TryRemove("videoChat", out string videoChat))
            {
        ...
        public async void DeleteChat()
        {
            ...
            // The Linux stand-in used to be a plain "are you sure?" with check hardcoded to false,
            // which silently dropped "also delete for X" and "stop the bot". DeleteChatPopup is in
            // the subset now, so both platforms ask the same question again.
        ...
        public void AddToContacts()
        {
            ...
            // The `#if !LINUX` that used to wrap this line is gone: UserEditPage is in the subset
            // since parcel 3. This body is the one ChatActionBarView's "Add contact" button calls
            // (ChatActionBarView.xaml.cs:77/81/119, none of them fenced), so on Linux that button
            // has been drawn and dead on every non-contact chat since the action bar was ported.
        ...
        // CERRADO 2026-08-27 (PARIDAD M22). El mismo calendario que ChatView.Date_Click, por la
        // otra puerta: el boton de la barra de busqueda del chat, que Controls/Chats/
        // ChatSearchBar.xaml enlaza con x:Bind.
        // Aviso para no leer esto de mas: HOY ESE BOTON NO EXISTE en esta cabeza. El csproj no
        // compila Controls/Chats/ChatSearchBar.xaml -- lo sustituye el stub de
        // Telegram.Linux/Xaml/ChatViewStubs.cs, que es un Control pelado con Update() vacio (ver
        // PARIDAD A5) --, asi que JumpDate no tiene hoy ningun llamante y ningun boton mentia por
        // aqui. Se implementa igual porque es el mismo cuerpo que Date_Click y porque la barra de
        // busqueda es lo siguiente que entra; cuando entre, el boton llega funcionando.
        ...
        private async Task ReportAsync(Vector<long> messages)
        {
            ...
            // u-059: this whole method used to be `#if !LINUX` because ReportChatPopup was out of
            // the Linux subset. It is in now (Telegram.Linux csproj), so the guard came off -- same
        ...
        {
            // Was #if !LINUX in its entirety, which is what made the join-requests band a dead
            // button and why u-013 collapsed it. Un-fenced in u-022 together with the popup it
            // opens: ChatJoinRequestsPopup and ChatJoinRequestsViewModel entered the subset, so
            // there is nothing left to fence off.
