    ...
    public partial class StickersViewModel : ViewModelBase
    {
...
#if LINUX
        // Uno hands the source to the items collection as
        // `Items.SetItemsSource(UnwrapItemsSource() as IEnumerable)`
        // (Uno.UI 6.6.184, ItemsControl.OnItemsSourceChanged), and `UnwrapItemsSource` only ever
        // unwraps a CollectionViewSource. So an ItemsSource that is NOT an IEnumerable does not
        // degrade into "one item": it becomes NO items, with no error and no warning.
        // The single-set arm below hands the GridView a StickerSetViewModel, which is a plain
        // object with a Stickers property (that property is what CollectionViewSource.ItemsPath
        // resolves in the many-sets arm, not a collection the control itself can walk). That is
        // why the pack preview opened with an EMPTY body while its title, its "..." menu and its
        // "Remove N stickers" button -- all bound to other properties -- were right.
        // Hand it the set's own collection instead. Nothing else changes: with a single set the
        // group header is collapsed anyway (OnChoosingGroupHeaderContainer), and the items are the
        // same StickerViewModel instances the grouped view would have yielded.
        // Left under #if because the Windows arm cannot be measured from here: whatever WinUI does
        // with a non-iterable ItemsSource, it is not this, and the port does not change it blind.
        public object ItemsView
        {
            get
            {
                if (Items.Count == 1)
                {
                    var stickers = Items[0].Stickers;
                    Logger.Info($"StickersPopup: single set, binding the grid straight to its {stickers?.Count ?? 0} stickers");
                    return stickers;
                }

                return ItemsSource.View;
            }
        }
#else
