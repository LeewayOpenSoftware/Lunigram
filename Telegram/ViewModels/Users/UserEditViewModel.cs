...
namespace Telegram.ViewModels.Users
{
    public partial class UserEditViewModel : ViewModelBase, IDelegable<IUserDelegate>, IHandle
    {
        ...
        // The two entries below are the only members of this page whose destination is not in the
        // Linux subset: UserAffiliatePage and ChatStarsPage are both unported screens. They are
        // reachable ONLY from UsernamePanel, which UserEditPage.xaml declares x:Load="False" and
        // only ever loads for a bot you own -- and on Linux the page collapses both buttons there
        // (see the #if LINUX in UserEditPage.xaml.cs's UpdateUser), so nothing draws a door into
        // these bodies. The guard is what makes the file compile at all: naming a type that is not
        // in the subset is CS0246, not a dead call.
        public void OpenAffiliate()
        {
#if LINUX
            Logger.Warning("UserEditViewModel: UserAffiliatePage is not in the Linux subset.");
#else
        ...
        {
#if LINUX
            Logger.Warning("UserEditViewModel: ChatStarsPage is not in the Linux subset.");
#else
