...
#if LINUX
            // There is no helper process to launch or kill: the icon is a StatusNotifierItem inside
            // this very process (see Telegram.Linux/Platform/DBus/StatusNotifierItem.cs).
            await DesktopIntegration.SetTrayVisibleAsync(value);
#else
            if (value)
            {
