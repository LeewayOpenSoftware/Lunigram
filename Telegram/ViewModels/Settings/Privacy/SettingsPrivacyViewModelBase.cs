...
namespace Telegram.ViewModels.Settings
{
    public partial class SettingsPrivacyViewModelBase : MultiViewModelBase, IHandle
    {
        ...
        // CC-2: CC-1 ported ChooseChatsPopup and ChatShareCell. Both methods below use
        // Session.Resolve<ChooseChatsViewModel>() + PickAsync -- the simple picker surface, not
        // the Configuration/SendExecute dispatch -- so neither depends on any of the 6 configs
        // CC-1 left guarded. The two rows that call them un-collapse across every privacy page in
        // the same tanda (SettingsPrivacyPageEx.HideExceptions).
        // u-018b (post-CC-3): swapped off the Obsolete popup.Legacy(Session) wrapper onto the
        // same DataContext resolution its real (non-Legacy) factory methods use -- no behavior
        // change, just the last 2 CS0612 warnings cleared now that Legacy() has no other callers.
        //
        // Nothing is lost from what the page SHOWS: the exception COUNT is computed by
        // UpdatePrivacyImpl from the rules TDLib returns, so both the per-page badges
        // (AllowedBadge / RestrictedBadge) and the "(-3, +12)" suffix on the root page's badge
        // stay correct and stay visible. What is lost is editing the list.
        public async void Always()
        {
            ...
            popup.DataContext = Session.Resolve<ChooseChatsViewModel>();
