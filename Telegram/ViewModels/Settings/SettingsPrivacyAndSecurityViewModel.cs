...
#if LINUX
        // The row that reaches this is collapsed in the page's constructor; the method has to
        // survive because the XAML binds Click="{x:Bind ViewModel.Passkeys}" and x:Bind is
        // resolved at compile time. BridgeApplicationContext (AppServiceConnection +
        // FullTrustProcessLauncher against the Telegram.Stub helper process) and the two
        // SettingsPasskeys* views it needs are not portable.
        public void Passkeys()
        {
            Logger.Info("Passkeys are not available on Linux");
        }
#else
