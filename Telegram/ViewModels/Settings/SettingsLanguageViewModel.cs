    ...
    public partial class SettingsLanguageViewModel : ViewModelBase, IDiffHandler<LanguagePackInfo>
    {
        ...
        public async void Change(LanguagePackInfo info)
        {
            ...
            if (response is Ok)
            {
                ...
                {
#if !LINUX
                    // Windows.ApplicationModel.Resources.Core does not exist in Uno: the two
                    // call sites of this same pair inside the subset (Common/MessageHelper.cs and
                    // Navigation/Services/FrameFacade.cs) already carry the guard, and so does
                    // MessageHelper's using directive. Nothing is lost by skipping it here: the
                    // .resw catalogue this resets holds only the English fallback, and every
                    // string the UI shows after a language change comes from TDLib's language
                    // pack through LocaleService.GetString / Client.Execute(GetLanguagePackString),
                    // which SetLanguageAsync has already re-pointed by this line.
