        ...
        public async void Create(IEnumerable<User> users)
        {
            ...
            if (peers.Length <= maxSize)
            {
                ...
                if (response is CreatedBasicGroupChat chat)
                {
                    ...
                    {
#if LINUX
                        // ChatInviteFallbackPopup (the "invite them by link instead" list) is
                        // outside the subset. The Linux member picker is not in either, so the
                        // list this popup would show is always empty here; the toast is the
                        // honest report if the server ever answers otherwise.
                        Logger.Info($"CreateNewBasicGroupChat: {chat.FailedToAddMembers.FailedToAddMembersValue.Count} member(s) could not be added");
                        ShowToast(Strings.InviteToGroupError);
#else
...
#if LINUX
            // Upstream never fills Preview, so on Windows picking a photo leaves the avatar
            // showing the initials. Here that would read as a dead button.
