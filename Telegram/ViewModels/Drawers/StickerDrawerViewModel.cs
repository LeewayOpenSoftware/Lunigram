    ...
    public partial class StickerDrawerViewModel : ViewModelBase
    {
        ...
        public async void Update(Chat chat)
        {
...
#if LINUX
            // The whole body below is guarded on BOTH calls returning Stickers, and there is no
            // else: if either answers an Error, SavedStickers is never filled and the drawer is
            // empty with nothing in the log to say why. This is the line that says which.
            Logger.Info($"sticker drawer data: favorite={result1?.GetType().Name ?? "null"} recent={result2?.GetType().Name ?? "null"} installed={result4?.Count().ToString() ?? "null"}");
#endif
            ...
            if (result1 is Stickers favorite && result2 is Stickers recent)
            {
                ...
                {
                    total += set?.Stickers?.Count ?? 0;
                }

                Logger.Info($"sticker drawer data: filled SavedStickers #{SavedStickers.GetHashCode()} with {stickers.Count} set(s), {total} sticker(s), vm #{GetHashCode()}");
#endif
            }
        }
