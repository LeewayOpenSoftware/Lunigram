        ...
        private async void SearchImpl(CancellationToken token)
        {
            if (IsPremium)
            {
                ...
                else if (response is ErrorStarsNeeded)
                {
                    State = SearchPostsState.Empty;
#if !LINUX
                    // The only line of this view model that reaches outside the subset: BuyPopup is
                    // Views/Stars/Popups, which is not in it. The state above is still set, so the
                    // tab says "no results" instead of buying stars.
