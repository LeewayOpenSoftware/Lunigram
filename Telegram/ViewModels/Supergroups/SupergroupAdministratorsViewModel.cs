...
#if !LINUX
// Views/Supergroups/Popups is empty in the Linux subset, so the namespace does not exist and the
// using itself would not compile - the using, not the calls that need it.
...
namespace Telegram.ViewModels.Supergroups
{
    public partial class SupergroupAdministratorsViewModel : SupergroupMembersViewModelBase, IHandle
    {
        ...
        public void EventLog()
        {
            ...
            {
#if LINUX
                // ChatEventLogPage is the recent-actions log: its own page, its own view model and
                // the whole message-rendering path reused for event messages. Out of the subset;
                // the row stays so the screen matches Unigram.
                Logger.Warning("SupergroupAdministratorsViewModel.EventLog: ChatEventLogPage is out of the Linux subset");
#else
        ...
        public void Add()
        {
            ...
            {
#if LINUX
                // SupergroupChooseMemberPopup is the member picker; out of the subset.
                Logger.Warning("SupergroupAdministratorsViewModel.Add: SupergroupChooseMemberPopup is out of the Linux subset");
#else
...
#if LINUX
            // The admin-rights editor; see SupergroupMembersViewModel.PromoteMember.
            Logger.Warning("SupergroupAdministratorsViewModel.EditMember: SupergroupEditAdministratorPopup is out of the Linux subset");
#else
