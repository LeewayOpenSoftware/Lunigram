...
namespace Telegram.ViewModels.Profile
{
    ...
    public abstract partial class ProfileTabsViewModel : MediaTabsViewModelBase, IHandle
    {
        ...
        protected async Task UpdateSharedCountAsync(Chat chat, IList<ProfileTabItem> tabs)
        {
            ...
            // u-093/F4: these 8 GetChatMessageCount round trips were previously awaited one at a
            // time - each one is its own TDLib actor request, so opening a profile paid for 8
            // round trips back to back instead of the one the slowest of them actually costs.
            // Firing them together doesn't reorder anything visible: responses[i] still lines up
            // with filters[i], so the tabs get added in the same order as before.
            var tasks = new Task<Count>[filters.Length];
            for (int i = 0; i < filters.Length; i++)
            {
                tasks[i] = GetCountAsync(filters[i]);
            }
