...
#if !LINUX
// Views/Stars/Popups is not part of the Linux subset, and it is the USING that fails, not the
// call. ReceivedGiftPopup (the gift detail sheet) lives there; every use is guarded below.
...
namespace Telegram.ViewModels.Profile
{
    public partial class ProfileGiftsTabViewModel : ViewModelBase, IHandle, IDiffHandler<ReceivedGift>
    {
        ...
        public void ShareCollection(GiftCollectionViewModel collection)
        {
            ...
            {
#if LINUX
                Logger.Warning("ProfileGiftsTabViewModel.ShareCollection: ChooseChatsPopup is not in the Linux subset.");
#else
        ...
        {
#if LINUX
            // ChooseGiftsPopup is the picker itself, so there is nothing to add without it.
            Logger.Warning("ProfileGiftsTabViewModel.AddGiftsToCollection: ChooseGiftsPopup is not in the Linux subset.");
            return;
#else
...
#if LINUX
            Logger.Warning("ProfileGiftsTabViewModel.OpenGift: ReceivedGiftPopup is not in the Linux subset.");
#else
        ...
        public void ShareGift(ReceivedGift gift)
        {
            ...
            {
#if LINUX
                Logger.Warning("ProfileGiftsTabViewModel.ShareGift: ChooseChatsPopup is not in the Linux subset.");
#else
        ...
        {
#if LINUX
            // ChooseChatsPopup is the recipient picker: without it there is nobody to transfer to.
            Logger.Warning("ProfileGiftsTabViewModel.TransferGift: ChooseChatsPopup is not in the Linux subset.");
#else
