...
#if LINUX
        /// <summary>
        /// Sends a voice note that is already Ogg/Opus, which on Linux is every one of them: the
        /// recorder encodes while it captures (Telegram.Linux/Platform/ChatRecordEngine.cs), so
        /// there is nothing to convert on the way out.
        /// </summary>
        /// <remarks>
        /// The Windows overload takes a <c>StorageFile</c> and a <c>ConversionType</c> and hands
        /// them to <c>ToGeneratedAsync</c>. That call answers <c>InputFileLocal(path)</c> for
        /// <c>ConversionType.Copy</c> with no arguments -- exactly what is built here -- so this is
        /// the same message, minus a round trip through <c>GenerationService</c>, which is out of
        /// the subset (it is MediaEncodingProfile and MediaTranscoder from end to end).
        /// </remarks>
        public async Task SendVoiceNoteAsync(string path, TimeSpan duration, byte[] waveform, FormattedText caption, MessageSelfDestructType selfDestructType)
        {
            var options = await PickMessageSendOptionsAsync();
            if (options == null)
            {
            ...
            var reply = GetReply(true);
            var input = new InputMessageVoiceNote(new InputVoiceNote(new InputFileLocal(path), (int)Math.Round(duration.TotalSeconds), waveform), caption, selfDestructType);
