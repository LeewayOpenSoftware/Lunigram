...
namespace Telegram.ViewModels
{
    public partial class DialogViewModel : IHandle
    {
        ...
        // UpdateForumTopicReadOutbox is published by ForumTopicService (live in the subset) and
        // Handle(UpdateChatReadOutbox) above deliberately skips forum topics (_forumTopic == null),
        // so with this off the outgoing read state never refreshed inside a topic. TopicListViewModel
        // already subscribes to the same update.
        ...
        // Both updates are Unigram's own (ForumTopicService / FeedbackChatTopicService, both in
        // the subset and both publishing today); only the two handlers and their subscriptions
        // were off, so a draft changed on another device never reached the composer of a forum
        // topic or of a direct-messages topic.
                    ...
                    else
                    {
                        ...
                        if (update == null || update(message))
                        {
                            ...
                            if (action1 != null)
                            {
                                ...
                                {
                                    if (action1 != null && container.ContentRoot() is MessageSelector selector && selector.Content is MessageBubble bubble)
                                    {
                                        action1(bubble, message);
                                    }
                                    else if (action2 != null && container.ContentRoot() is MessageService service)
                                    {
