...
namespace Telegram.Entities
{
    public partial class Country
    {
        ...
        {
#if LINUX
            // Uno does not implement GeographicRegion.IsSupported; ICU's RegionInfo knows the same names.
            try
            {
                var region = new System.Globalization.RegionInfo(code);
                return string.IsNullOrEmpty(region.DisplayName) ? englishName : region.DisplayName;
            }
            catch
            {
                // Unknown or synthetic codes (FT, XK...) and invariant globalization mode.
            }
#else
