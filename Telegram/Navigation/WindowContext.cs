...
namespace Telegram.Navigation
{
    ...
    public partial class WindowContext
    {
        ...
        #if LINUX
        private void OnShutdownStarting(DispatcherQueue sender, Microsoft.UI.Dispatching.DispatcherQueueShutdownStartingEventArgs args)
#else
...
#endif
        {
            ...
            #if LINUX
            if (!sender.TryEnqueue(Microsoft.UI.Dispatching.DispatcherQueuePriority.Low, OnShutdownDrain))
#else
...
#if LINUX
                // Only VoipWindow is in this port's subset; the group-call windows are not.
                IsCallInProgress = value is VoipWindow;
#else
...
#if LINUX
            // This head has its own RootPage and applies the material from ApplyContentMaterial
            // in WindowContext.Linux.cs; none of the other window types are in the subset.
#else
...
#if LINUX
            // The verification harness. This block was lost in the 12.10.2 merge: every one of
            // these is scheduled from here and nowhere else, so without it the probes never arm --
            // and because OnLoaded only fires once the content is really in the tree, it is also
            // the second layer of the SetHostContent bug rather than a separate one.
            VisualTreeDump.Schedule(_window);
            TypeTest.Schedule(_window);
            ScrollTest.Schedule(_window);
            PointerTest.Schedule(_window);
            PointerTest.ScheduleRequests(_window);
            HistoryProbe.Schedule(_window);
            NotifyTest.Schedule(_window);
            CallTest.Schedule(_window);
            LiveTest.Schedule(_window);
            MediaTest.Schedule(_window);
            BankSetup.Schedule(_window);
            BankSetup.ScheduleTeardown(_window);
            Screenshot.Schedule(_window);
            Screenshot.ScheduleRequests(_window);
#endif
        ...
        public async void Activate(IActivatedEventArgs args, INavigationService service, AuthorizationState state)
        {
            ...
            {
                // A page that fails to build on Uno would otherwise leave the window blank silently.
...
#if LINUX
        /// <summary>
        /// Protocol activation from outside the process: a <c>tg://</c> link handed over by another
        /// instance, a desktop action, a notification. On Windows the same string arrives wrapped in
        /// an <c>IActivatedEventArgs</c> that only the platform can build, so this is the way in to
        /// the shared handling below. Must be called on the window's dispatcher.
        /// </summary>
        public void ActivateArguments(string arguments)
        {
            var service = NavigationServices.GetByFrameId($"{LifetimeService.Current.ActiveItem.Id}")
                ?? NavigationServices.FirstOrDefault();

            if (service == null)
            {
                // Nothing to navigate with yet. The caller keeps the window in front, which is the
                // part of the activation that still makes sense.
                Logger.Warning("No navigation service for the activation");
                return;
            }

            Activate(arguments, service);
        }
#endif
...
#if LINUX
            // Only launch activation exists on Linux (no protocol, file or toast activation yet).
            var launch = args as Windows.ApplicationModel.Activation.LaunchActivatedEventArgs;
        ...
        public static IntPtr GetWindowHandle(XamlRoot xamlRoot)
        {
            var context = ForXamlRoot(xamlRoot);
#if LINUX
            // No HWND on this host; callers use it only for WinRT interop, which Uno drives itself.
            return IntPtr.Zero;
#else
