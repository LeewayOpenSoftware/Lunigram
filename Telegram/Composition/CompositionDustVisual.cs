...
namespace Telegram.Composition
{
    ...
    public abstract partial class CompositionDustVisual
    {
        ...
        {
#if LINUX
            // Capture needs a frozen visual surface, which Uno Skia has no way to make: a deleted
            // message goes without its send-off for now.
            return null;
#else
...
#if LINUX
            // No ICompositionVisualSurfacePartner (Windows-only COM) to freeze the surface with.
            return false;
#else
            // 12.10.2 folded the NET9/older split into surface.TryGetPartner, so what used to be
            // three branches here is two: this head, and everything else.
