...
#if LINUX
        /// <summary>
        /// Takes the whole blob tree off screen, the small circle included.
        /// </summary>
        /// <remarks>
        /// <para><see cref="Clear"/> only zeroes the two path blobs. The small one is an ellipse,
        /// its constructor leaves it at <c>Scale = minScale</c> (0.45), and nothing here ever moves
        /// it again - <c>CompositionBlobShape.Level</c> would, but it animates a
        /// <c>CompositionShape</c>, which Uno cannot animate at all (no
        /// <c>SetAnimatableProperty</c> override, and <c>Compositor.RegisterAnimation</c> returns
        /// early unless the target <c>is Visual</c>). So on Linux a filled circle of radius 36 was
        /// left sitting at the tree's own (80, 80) with nobody scaling it, which in
        /// <c>ChatRecordBar</c> - where the host border is deliberately shrunk to 80 px - is the
        /// bottom-right corner of the button rather than its middle.</para>
        /// <para>Written, not animated, and the animation is stopped first: a KeyFrameAnimation
        /// that has already finished still owns its property in Uno and swallows the assignment
        /// (PORTING.md section 6).</para>
        /// </remarks>
...
#if LINUX
                // CreateSmoothCurve returns the port's own CompositionPath (Telegram.Linux/
                // Graphics/CompositionPath.cs shadows Uno's by name and namespace, because the
                // Skia compositor only draws its own internal geometry source). The Path setter
                // is typed on Uno's class, so it has to go through the SetPath extension that
                // exists for exactly this assignment.
                _shapeLayer.SetPath(compositor.CreateSmoothCurve(points, _smoothness));
#else
