...
namespace Telegram.Views.Folders
{
    ...
    public sealed partial class FolderPage : HostedPage
    {
        ...
        {
#if LINUX
            ProfileTabContainer.Bind<FontIcon>(sender, args, null, (textBlock, item) =>
            {
                if (item is ChatFolderIcon2 icon)
                {
        ...
        {
#if LINUX
            ProfileTabContainer.Bind<Grid>(sender, args, null, (content, item) =>
            {
                if (content.Children[0] is TextBlock textBlock && item is NameColor colors)
                {
        ...
        {
#if LINUX
            TypedEventHandler<UIElement, ContextRequestedEventArgs> contextHandler;
            if (sender == IncludeHost)
            {
                contextHandler = Include_ContextRequested;
            }
            else if (sender == ExcludeHost)
            {
                contextHandler = Exclude_ContextRequested;
            }
            else
            {
                contextHandler = Link_ContextRequested;
            }
            if (sender == ScrollingHost)
            {
                ProfileTabContainer.Bind<ChatInviteLinkCell>(sender, args, contextHandler, (chatInviteLinkCell, item) =>
                {
                    if (item is ChatFolderInviteLink inviteLink)
                    {
            ...
            {
                ProfileTabContainer.Bind<ProfileCell>(sender, args, contextHandler, (profileCell, item) =>
                {
                    if (item is ChatFolderElement element)
                    {
