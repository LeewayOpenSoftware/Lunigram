...
namespace Telegram.Views.Folders
{
    public sealed partial class FoldersPage : HostedPage
    {
        ...
        public FoldersPage()
        {
            ...
            AutomationProperties.SetName(TabsViewOnTopRadio, Strings.TabsViewOnTop);
            AutomationProperties.SetName(TabsViewOnLeftRadio, Strings.TabsViewOnLeft);
        ...
        {
#if LINUX
            TypedEventHandler<UIElement, ContextRequestedEventArgs> contextHandler = sender == ScrollingHost ? Item_ContextRequested : null;
            ProfileTabContainer.Bind<Grid>(sender, args, contextHandler, (content, item) =>
            {
                if (item is ChatFolderInfo folder)
                {
                ...
                else if (item is RecommendedChatFolder recommended)
                {
