...
#if LINUX
            // Same as the drawers: ContentTemplateRoot never answers on Uno, so this whole branch
            // would be skipped and the folder-invite list would draw empty rows. IsEnabled is set
            // synchronously because the row has to refuse the click from the first frame, not from
            // whenever the cell materialises.
            else if (args.Item is Chat chat)
            {
                args.ItemContainer.IsEnabled = ViewModel.CanBeAdded(chat);

                var selected = args.ItemContainer.IsSelected;
                var service = ViewModel.ClientService;

                DrawerContainers.WithContentRoot<ChatShareCell>(sender, args.ItemContainer, args.Item, content =>
                {
                    content.UpdateState(selected, false, true);
                    content.UpdateSharedChat(service, args, OnContainerContentChanging);
                });
