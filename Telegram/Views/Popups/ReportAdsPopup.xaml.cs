...
namespace Telegram.Views.Popups
{
    ...
    public sealed partial class ReportAdsPopup : ContentPopup
    {
        ...
        private void OnContainerContentChanging(ListViewBase sender, ContainerContentChangingEventArgs args)
        {
            ...
            // template IS applied, but the property that hands you its root is not. Extensions
            // .ContentRoot() walks to the real root instead; same substitute the chat menu and the
            // drawers already use. Without this the row's TextBlock is never filled and every
            // report option draws blank.
            else if (args.ItemContainer.ContentRoot() is Grid content && args.Item is ReportOption option)
#else
        ...
        private void ShowHideBackButton(bool show)
        {
            ...
            {
#if LINUX
                // PORTING.md §6: a KeyFrameAnimation that has ALREADY FINISHED still owns its
                // property on Uno -- Compositor.RenderRootVisual re-evaluates it every frame, so
                // the InsertVector3 below is overwritten and the title stays shifted by -28px.
                // WinUI's default StopBehavior (LeaveCurrentValue) is why the shared code can
                // write over it without thinking. Stopping it first is the whole fix.
                visual2.StopAnimation("Translation.X");
#endif
