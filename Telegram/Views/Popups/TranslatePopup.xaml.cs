    ...
    public sealed partial class TranslatePopup : ContentPopup
    {
        ...
        private async void OnOpened(ContentDialog sender, ContentDialogOpenedEventArgs args)
        {
            ...
            else
            {
                ...
                {
#if LINUX
                    // El mensaje de vista instantanea (MessageRichMessage) no lo dibuja este port
                    // -- InstantContent es un cascaron -- y su traduccion pasa por
                    // PageBlockHelper.ToInputRichMessage, que la reduccion Linux de ese helper
                    // (Telegram.Linux/Hubs/PageBlockHelper.cs) no trae: el fichero de upstream
                    // esta escrito contra un esquema de TDLib mas nuevo que el vendorizado.
                    // Aqui _message es siempre null, porque el unico llamante Linux
                    // (DialogViewModel.TranslateMessage) deja esa rama fuera; esta guarda existe
                    // para que el fichero compile, no para cubrir un camino vivo.
                    _loadingMore = false;
                    return;
#else
