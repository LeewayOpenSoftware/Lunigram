...
#if LINUX
            // The RecentUserHeadChanged="..." attribute is win:-only in the XAML, and the
            // subscription moves here. Reason, measured on 2026-08-27 while linking this popup in:
            // Uno's XamlCodeGenerator emits the handler cast with the delegate's parameter types
            // taken VERBATIM from the C# declaration, unqualified --
            // `(global::Telegram.Controls.ProfilePicture sender, global::MessageSender ...)` --
            // and `RecentUserHeadChangedHandler(ProfilePicture, MessageSender)` names a type that
            // only resolves through `using Telegram.Td.Api`. The generated file has no such using,
            // so it fails with CS0400 on `global::MessageSender`. It is not a defect of this
            // popup: any XAML in this head that wires an event whose delegate mentions an
            // unqualified type from another namespace will hit it (there are four more such
            // attributes on RecentUserHeads alone). The += below is what WinUI's compiler would
            // have written, and it costs nothing.
            Participants.RecentUserHeadChanged += Participants_RecentUserHeadChanged;
#endif
