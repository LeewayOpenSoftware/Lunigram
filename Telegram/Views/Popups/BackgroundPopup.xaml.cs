...
namespace Telegram.Views.Popups
{
    public sealed partial class BackgroundPopup : ContentPopup, IBackgroundDelegate
    {
...
#if LINUX
            // The preview is painted by ConvertBackground, which is not a converter at all: it
            // returns null every time and its real work is the UpdateSource call inside it. That
            // makes the whole preview a SIDE EFFECT of an x:Bind evaluation, and an x:Bind
            // evaluation is not something the view model can ask for. The generated binding
            // (verified in the assembly: _component_5, path "ViewModel.Item") is refreshed by
            // Bindings.Update() on Loading and thereafter only when the path notifies - and the
            // ROOT of that path is `ViewModel => DataContext as BackgroundViewModel`, a plain CLR
            // property with nothing to notify with. Evaluate it once while DataContext is still
            // unassigned and TryGetInstance returns false, and there is no second chance.
            //
            // This is the same delegate call the popup already trusts for every other piece of
            // its chrome, it arrives with the very Background the preview needs, and the view
            // model raises it explicitly after its batch update. Painting from here does not
            // replace the binding; it removes the preview's dependence on WHEN the binding runs.
            Preview.XamlRoot ??= XamlRoot;
            Preview.UpdateSource(ViewModel.ClientService, wallpaper, false);

            Logger.Debug(string.Format("BackgroundPopup: preview painted from the delegate, background {0}", wallpaper.Id));
#endif
...
#if LINUX
                Logger.Debug(string.Format("BackgroundPopup: preview painted from the x:Bind converter, background {0}", background.Id));
#endif

                PatternList.ForEach<Document>((container, document) =>
                {
                    // ContentRoot() answers null on Uno whenever the container has not entered the
                    // visual tree yet (PORTING.md section 6; eb0d14c is the same call, one page
                    // over). Unguarded it throws, and the throw happens INSIDE a binding
                    // evaluation, where it is swallowed - so every pattern after the first missing
                    // container is skipped silently, and nothing anywhere says why.
                    if (container.ContentRoot() is not ChatBackgroundPresenter content)
                    {
                        Logger.Debug("BackgroundPopup: pattern container has no ContentRoot yet, skipping it");
                        return;
                    }
        ...
        private void OnContainerContentChanging(ListViewBase sender, ContainerContentChangingEventArgs args)
        {
            ...
            else if (args.ItemContainer.ContentRoot() is ChatBackgroundPresenter content && args.Item is PatternInfo pattern)
            {
