...
#if !LINUX
                // GlobalizationPreferences.HomeGeographicRegion is one more of the members Uno
                // marks [NotImplemented] for __SKIA__, and every one of them THROWS rather than
                // returning a default (measured on 2026-08-27 through CalendarPopup, which died in
                // its constructor on the sibling member Calendars). The region only picks the
                // decimal separator, and the culture-less DecimalFormatter below already reads it
                // from the current culture, so what is lost here is nothing at all.
