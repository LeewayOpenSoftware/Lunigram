...
namespace Telegram.Views.Popups
{
    ...
    public sealed partial class ReportChatPopup : ContentPopup
    {
        ...
        private void OnContainerContentChanging(ListViewBase sender, ContainerContentChangingEventArgs args)
        {
            ...
            {
                // The reason is written from Loaded as well because ContainerContentChanging runs
                // before Uno has expanded the item template, and until it has, nothing can be
                // found under the container -- with no exception to show for it, which is what
                // left every row of the list blank. See the ContentTemplateRoot entry in
                // Telegram.Linux/PORTING.md. Containers are recycled, so the handler is detached
                // before it is attached and the option is re-read from the container rather than
                // captured here.
        ...
        private void OnContainerLoaded(object sender, RoutedEventArgs e)
        {
            ...
            {
                Logger.Warning("No TextBlock under the container of a report option");
            }
        }

        private static bool UpdateContainer(SelectorItem container)
        {
            if (container.ContentRoot() is Grid content && container.Content is ReportOption option
                && content.Children.Count > 0 && content.Children[0] is TextBlock textBlock)
            {
