...
namespace Telegram.Views.Popups
{
    ...
    public sealed partial class ChooseChatsPopup : ContentPopup
    {
        ...
        public ChooseChatsPopup()
        {
            ...
            // {CustomResource} on an attached property (AutomationProperties.Name,
            // ToolTipService.ToolTip) does not resolve under Uno Skia (PORTING.md §6): the
            // generated code writes it as a direct property of the target element, which neither
            // Name nor ToolTip is on a Button. The value is static, so setting it here once,
            // unconditionally, is equivalent on both platforms to what the markup extension would
            // have produced on Windows -- unlike u-056's ContactsSortedByHeader.xaml fix for the
            // same trap, this is NOT `#if LINUX`-only, so Windows keeps the accessible name too.
            Microsoft.UI.Xaml.Automation.AutomationProperties.SetName(Alias, Strings.SelectAccount);
            Microsoft.UI.Xaml.Controls.ToolTipService.SetToolTip(Alias, Strings.SelectAccount);
            Microsoft.UI.Xaml.Automation.AutomationProperties.SetName(EmojiButton, Strings.Emoji);
            Microsoft.UI.Xaml.Automation.AutomationProperties.SetName(Send, Strings.Send);
            Microsoft.UI.Xaml.Controls.ToolTipService.SetToolTip(Send, Strings.Send);
        }
...
#if !LINUX
                    // ScheduleMessageToast (a TeachingTip-based date/time picker) is not in the
                    // Linux subset; porting it is its own parcel. "Send when online" above needs
                    // no popup (it just sets a TDLib enum) and stays live.
        ...
        {
#if LINUX
            ProfileTabContainer.Bind<ChatShareCell>(sender, args, null, (content, item) =>
            {
                if (item is FolderFlag folder)
                {
...
#if LINUX
            // ContentTemplateRoot is always null on Uno (PORTING.md §6); ProfileTabContainer.Bind
            // takes its place, same recipe as ProfileGroupsTabPage/ProfileCell.Linux.cs. ChatsPanel
            // and ForumList share this one handler but each has a single, fixed ItemTemplate, so
            // `sender` -- not the container's content -- is what tells them apart.
            else if (sender == ChatsPanel)
            {
                ProfileTabContainer.Bind<ChatShareCell>(sender, args, null, (content, item) =>
                {
                    content.UpdateState(args.ItemContainer.IsSelected, false, true);
                    content.UpdateChatInflated(ViewModel.ClientService, item as Chat);
                });
            }
            else if (sender == ForumList)
            {
                ProfileTabContainer.Bind<ForumTopicShareCell>(sender, args, null, (content, item) =>
                {
                    if (item is ForumTopic forumTopic)
                    {
                        content.UpdateCell(ViewModel.ClientService, forumTopic);
                    }
                    else if (item is DirectMessagesChatTopic directMessagesChatTopic)
                    {
                        content.UpdateCell(ViewModel.ClientService, directMessagesChatTopic);
                    }
                });
            }
#else
        ...
        {
#if LINUX
            // RE-3, measured in the app: Uno's WinUI Window has no CoreWindow, so this line threw
            // a NullReferenceException on EVERY open of the share/forward popup -- straight out of
            // Loaded, where nothing catches it, so it reached the WatchDog and the handler never
            // ran.
            //
            // u-charrecv-b: the substitute RE-3 was waiting for exists now. Same pattern as
            // ChatView.xaml.cs (u-19, Stanley): tunnel on PreviewKeyDown, read the character Uno
            // decoded, focus the target WITHOUT inserting -- the box types the keystroke itself
            // once it has the focus, because the tunnel runs before delivery.
            //
            // On `this` rather than on the window's content, which is where ChatView subscribes.
            // ChatView needs the window because a handler on ChatView itself is not on the route
            // when nothing inside the chat has the focus. A popup is the other way round: it is
            // hosted in the XamlRoot's popup layer, a different branch from the window content, so
            // a handler there would never see these keys -- and the case RE-3 described (a
            // printable character with the LIST focused) keeps the focus inside this popup.
        ...
        // delivers it. It applies to the typed-text branch AND to paste: the comment box pastes a
        // Ctrl+V itself once it has the focus, so pasting here as well would paste twice. Enter is
        // not text and cannot double, so it still accepts.
        private bool TryHandleReceivedCharacter(string character, bool insert = true)
        {
        ...
        {
            CharacterInputBridge.Claim(args, nameof(ChooseChatsPopup), TryHandleReceivedCharacter, "claiming it");
        }
#endif
...
#endif
                {
                    // SendMessagesView completes a Windows Share-contract request (OS share
                    // target progress UI); there is no such contract on Linux, so
                    // ChooseChatsConfigurationShareOperation's only producer, SharePage, is out of
                    // the subset too (u-072 recipe) and this branch is unreachable, not guarded
                    // around a loss.
                    ViewModel.SendCommand.Execute();
                }
            }
...
#if LINUX
        // CaptionInput is CaptionTextBox : TextBox here (Telegram.Linux/Xaml/Stubs/
        // CaptionTextBox.cs), not FormattedTextBox, so its Accept event carries that sender type
        // instead. sender is unused either way.
        private void CaptionInput_Accept(CaptionTextBox sender, EventArgs args)
#else
