...
namespace Telegram.Views.Authorization
{
    public sealed partial class AuthorizationPage : CorePage, ISignInDelegate
    {
        ...
        private async void Switch1_Click(object sender, RoutedEventArgs e)
        {
            ...
            // same object cannot drive two visuals at once: the inner panels get their own copies.
            var opacity4 = BootStrapper.Current.Compositor.CreateScalarKeyFrameAnimation();
            opacity4.InsertKeyFrame(0, 1);
            opacity4.InsertKeyFrame(1, 0);

            var opacity5 = BootStrapper.Current.Compositor.CreateScalarKeyFrameAnimation();
            opacity5.InsertKeyFrame(0, 0);
            opacity5.InsertKeyFrame(1, 1);

            inner1.StartAnimation("Opacity", opacity4);
            inner2.StartAnimation("Opacity", opacity5);
        ...
        private async void Switch2_Click(object sender, RoutedEventArgs e)
        {
            ...
            // same object cannot drive two visuals at once: the inner panels get their own copies.
            var opacity4 = BootStrapper.Current.Compositor.CreateScalarKeyFrameAnimation();
            opacity4.InsertKeyFrame(1, 1);
            opacity4.InsertKeyFrame(0, 0);

            var opacity5 = BootStrapper.Current.Compositor.CreateScalarKeyFrameAnimation();
            opacity5.InsertKeyFrame(1, 0);
            opacity5.InsertKeyFrame(0, 1);
