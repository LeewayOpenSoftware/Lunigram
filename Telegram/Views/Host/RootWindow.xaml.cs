...
namespace Telegram.Views.Host
{
    ...
    public sealed partial class RootWindow : WindowContent
    {
        ...
        private void UpdateContainerContent(SelectorItem container, object item)
        {
            ...
            {
                var content = container.ContentRoot() as Grid;
                if (content == null)
                {
                    return;
                }
        ...
        private void Navigation_PaneOpening(SplitView sender, object args)
        {
            ...
            // One animation instance per visual, deliberately: Uno's Compositor.RegisterAnimation
            // is a Dictionary.Add keyed by the animation, so starting the SAME instance on a
            // second visual throws ArgumentException. Here that exception escapes the PaneOpening
            // handler through the SplitView.IsPaneOpen setter and into Uno's pointer dispatch,
            // which stops the dispatcher for good: the app stays alive at 0% CPU, the pane never
            // opens and no timer ever fires again. Same shape as before on Windows, where sharing
            // one instance was legal but bought nothing.
            ScalarKeyFrameAnimation Opacity()
            {
                var animation = compositor.CreateScalarKeyFrameAnimation();
                animation.InsertKeyFrame(0, 0, ease);
                animation.InsertKeyFrame(1, 1, ease);
            ...
            Vector3KeyFrameAnimation Scale()
            {
                ...
                animation.InsertKeyFrame(0, new Vector3(0, 0, 0), ease);
                animation.InsertKeyFrame(1, new Vector3(1, 1, 0), ease);
                animation.Duration = TimeSpan.FromMilliseconds(350);
                return animation;
            }
        ...
        private void Navigation_PaneClosing(SplitView sender, SplitViewPaneClosingEventArgs args)
        {
            ...
            // See Navigation_PaneOpening: one instance per visual, or Uno throws on the second
            // StartAnimation and the exception takes the dispatcher with it.
            ScalarKeyFrameAnimation Opacity()
            {
                var animation = compositor.CreateScalarKeyFrameAnimation();
                animation.InsertKeyFrame(0, 1, ease);
                animation.InsertKeyFrame(1, 0, ease);
            ...
            {
                var animation = compositor.CreateVector3KeyFrameAnimation();
                animation.InsertKeyFrame(0, new Vector3(1, 1, 0), ease);
