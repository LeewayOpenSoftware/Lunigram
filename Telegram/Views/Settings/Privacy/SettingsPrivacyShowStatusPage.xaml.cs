...
#if LINUX
            // "Subscribe to Premium". SubscribeToPremium calls NavigationService.ShowPromo, which
            // is inert on Linux (Common/TLNavigationService.cs:114 logs "Premium promo is not
            // available on Linux" and returns) because the promo is a WebView2 screen. The row
            // would draw with its gradient star icon and do nothing on click, so the whole
            // HeaderedControl goes. Its Footer is only the explanatory text for that row.
            SettingsPrivacyPageEx.HideExceptions(this, () => PremiumRoot);
#endif
        }
