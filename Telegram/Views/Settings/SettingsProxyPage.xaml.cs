...
namespace Telegram.Views.Settings
{
    public sealed partial class SettingsProxyPage : HostedPage, INavigablePage
    {
        ...
        {
#if LINUX
            // ChoosingItemContainer (above) never fires on Uno and ContentTemplateRoot is always
            // null once the container has a template - the usual pair (PORTING.md 6), fixed with
            // the same Telegram.Common.ProfileTabContainer.Bind the profile tab lists use. It
            // re-attaches ContextRequested itself, so Proxy_ContextRequested does not need a
            // separate subscription here.
            ProfileTabContainer.Bind<RadioButton>(sender, args, Proxy_ContextRequested, (content, item) =>
            {
