...
#if LINUX
        // internal rather than public: FileType is built by the Telegram.Generators source
        // generator from td_api.tl, and Uno's BindableTypeProviders emits a bare
        // typeof(global::FileType) for such types, which is CS0400. Same defect, and same fix, as
        // SettingsLoginEmailAddressPopup.CodeInfo - see the long note there.
        internal IList<FileType> SelectedItems { get; private set; }
#else
...
#if LINUX
            // ContentTemplateRoot is always null in Uno, so this whole body - which is what puts
            // the file-type name, its size and its colour on each checkbox - would never run, and
            // the dialog would list a column of blank, unchecked boxes. Not phased upstream, so
            // there is nothing to inflate; only the lookup changes.
            else if (args.ItemContainer.ContentRoot() is CheckBox check && args.Item is StorageChartItem item)
            {
#else
