...
namespace Telegram.Views.Calls
{
    public sealed partial class LiveStreamWindow : WindowContent
    {
        ...
        private void ShowHideTransport(bool show)
        {
            ...
            // Uno: a KeyFrameAnimation instance is single-target, so each visual needs its own
            // (PORTING.md, "instancia compartida" rule) instead of sharing one across StartAnimation calls.
            ScalarKeyFrameAnimation CreateOpacity()
            {
                var animation = top.Compositor.CreateScalarKeyFrameAnimation();
                animation.InsertKeyFrame(0, show ? 0 : 1);
                animation.InsertKeyFrame(1, show ? 1 : 0);
                return animation;
            }

            top.StartAnimation("Opacity", CreateOpacity());
            top1.StartAnimation("Opacity", CreateOpacity());
            bottom.StartAnimation("Opacity", CreateOpacity());
            bottom1.StartAnimation("Opacity", CreateOpacity());
