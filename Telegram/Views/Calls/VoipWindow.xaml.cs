...
namespace Telegram.Views.Calls
{
    ...
    public sealed partial class VoipWindow : WindowContent
    {
...
#if LINUX
        private readonly VoipVideoRenderer _localRenderer;
        private readonly VoipVideoRenderer _remoteRenderer;
#endif
...
#if LINUX
            // P17-R1. VoipVideoOutput hangs a SpriteVisual off these two rectangles with
            // ElementCompositionPreview.SetElementChildVisual; on Skia that reaches the Linux shim
            // and paints nothing, so even a renderer would have had nowhere to draw. These put a
            // real element in the real tree, right on top of each rectangle and in its cell, and
            // drive it from the sink the output already owns.
            _localRenderer = VoipVideoRenderer.Attach(LocalVideo.Parent as Panel, LocalVideo, _localVideo.Sink);
            _remoteRenderer = VoipVideoRenderer.Attach(RemoteVideo.Parent as Panel, RemoteVideo, _remoteVideo.Sink);
#endif
...
#if !LINUX
            // Uno lanza NotSupportedException nada mas llamar a GetForCurrentView ("use
            // Window.AppWindow.Closing instead"), y esto se ejecuta en el constructor de la
            // pagina: sin la guarda, la pantalla de llamada no llega a aparecer. Medido con la
            // sonda UNIGRAM_CALL_TEST. Aqui hay una sola ventana y cerrarla ya pasa por el
            // camino de RootPage, asi que no se pierde nada mas que el aviso de cierre.
        ...
        {
            AutomationProperties.SetName(SettingsButton, Strings.VoipSettings);
            ToolTipService.SetToolTip(SettingsButton, Strings.VoipSettings);
        }
        ...
        {
            UpdateIncomingButtons(args.State == VoipState.Ringing && !_call.IsOutgoing);
        ...
        private bool? _incomingButtonsShown;

        private void UpdateIncomingButtons(bool ringing)
        {
            if (_incomingButtonsShown == ringing)
            {
                return;
            }

            _incomingButtonsShown = ringing;

            ActiveButtons.Visibility = ringing ? Visibility.Collapsed : Visibility.Visible;
            IncomingButtons.Visibility = ringing ? Visibility.Visible : Visibility.Collapsed;

            if (ringing)
            {
                var withVideo = _call.IsVideo;

                AcceptVideoButton.Visibility = withVideo ? Visibility.Visible : Visibility.Collapsed;
                AcceptVideoLabel.Visibility = withVideo ? Visibility.Visible : Visibility.Collapsed;
                AcceptVideoColumn.Width = withVideo ? GridLength.Auto : new GridLength(0);
            }
        }
        ...
        private void ShowHideVideoToPlain(bool show, UIElement element)
        {
            if (show)
            {
                ...
                visual1.Clip = null;
#if LINUX
                var rendererElement = RendererFor(element)?.Element;
                if (rendererElement != null)
                {
                    rendererElement.Visibility = Visibility.Visible;

                    var rendererVisual = ElementComposition.GetElementVisual(rendererElement);
                    rendererVisual.Scale = Vector3.One;
                    rendererVisual.Offset = Vector3.Zero;
                    rendererVisual.Opacity = 1;
                    rendererVisual.Clip = null;
                }
#endif
...
#if LINUX
        /// <summary>
        /// The renderer VoipVideoRenderer.Attach put on top of <paramref name="placeholder"/>, if
        /// any. The choreography below only ever moves/clips/animates the placeholder's visual -
        /// the renderer element sits at a fixed full-cell size on top of it (see Attach's own
        /// comment), so without this it always paints full-bleed regardless of which video is
        /// supposed to be the small PiP corner. Mirroring Visibility/CenterPoint/Clip/animations
        /// from the placeholder onto this keeps one choreography instead of two that can disagree.
        /// </summary>
        private VoipVideoRenderer RendererFor(UIElement placeholder)
        {
            if (ReferenceEquals(placeholder, LocalVideo))
            {
                return _localRenderer;
            }
            else if (ReferenceEquals(placeholder, RemoteVideo))
            {
        ...
        private void ShowHideCollapsedToMaximized(bool show, UIElement element, Vector2 frame)
        {
            ...
            photo.Clip = clip2;
#if LINUX
            Visual rendererVisual = null;
            if (rendererElement != null)
            {
                rendererVisual = ElementComposition.GetElementVisual(rendererElement);
                rendererVisual.CenterPoint = visual1.CenterPoint;
                rendererVisual.Clip = clip1;
            }
#endif
            ...
            // Background from small to big. Local functions, not shared instances: Uno keys
            // RegisterAnimation by the ANIMATION OBJECT, so starting the very same instance on a
            // second visual (visual1 here, rendererVisual below) throws there even though it is
            // legal on Windows - each StartAnimation call below gets its own.
            Vector3KeyFrameAnimation MakeScale1()
            {
                var animation = compositor.CreateVector3KeyFrameAnimation();
                animation.InsertKeyFrame(show ? 0 : 1, new Vector3(ratio1, ratio1, 0));
                animation.InsertKeyFrame(show ? 1 : 0, new Vector3(1, 1, 0));
                animation.Duration = duration;
                return animation;
            }

            Vector3KeyFrameAnimation MakeOffset1()
            {
                var animation = compositor.CreateVector3KeyFrameAnimation();
                animation.InsertKeyFrame(show ? 0 : 1, new Vector3(-offset, 0));
                animation.InsertKeyFrame(show ? 1 : 0, new Vector3(0));
                animation.Duration = duration;
                return animation;
            }

            ScalarKeyFrameAnimation MakeOpacity1()
            {
            ...
            visual1.StartAnimation("Scale", MakeScale1());
            visual1.StartAnimation("Offset", MakeOffset1());
            visual1.StartAnimation("Opacity", MakeOpacity1());
#if LINUX
            if (rendererVisual != null)
            {
                rendererVisual.StartAnimation("Scale", MakeScale1());
                rendererVisual.StartAnimation("Offset", MakeOffset1());
                rendererVisual.StartAnimation("Opacity", MakeOpacity1());
            }

            // CreatePathKeyFrameAnimation() throws on Uno/Skia (ApiInfo.CanAnimatePaths=false),
            // silently, mid-batch - the clip stayed at its empty default forever and this element
            // (the remote/local video visual in the solo-video path) never became visible. Same
            // substitute as ShowHideCollapsedToMinimized/OnSizeChanged: no path keyframes, just
            // jump the clip geometry straight to whichever end `show` lands on; Scale/Offset/
            // Opacity above still animate, so the jump reads as part of the same motion.
            geometry1.SetPath(show
                ? new CompositionPath(CanvasGeometry.CreateRoundedRectangle(null, 0, 0, ActualSize.X, ActualSize.Y, 0, 0))
                : new CompositionPath(CanvasGeometry.CreateRoundedRectangle(null, side + part, part, remoteMin, remoteMin, remoteMin, remoteMin)));
#else
...
#if LINUX
            // Same seam as geometry1 above, same substitute.
            geometry2.SetPath(show
                ? new CompositionPath(CanvasGeometry.CreateRoundedRectangle(null, 0, 0, Photo.ActualSize.X, Photo.ActualSize.Y, 0, 0))
                : new CompositionPath(CanvasGeometry.CreateRoundedRectangle(null, 0, 0, Photo.ActualSize.X, Photo.ActualSize.Y, Photo.ActualSize.Y / 2, Photo.ActualSize.Y / 2)));
#else
        ...
        private void ShowHideMinimizedToMaximized(bool show, UIElement element, Vector2 frame, VoipVideoActiveState collapsed)
        {
            ...
            {
                rendererElement.Visibility = Visibility.Visible;
                Canvas.SetZIndex(rendererElement, Canvas.GetZIndex(element));
            }

            var localRendererElement = _localRenderer?.Element;
            var remoteRendererElement = _remoteRenderer?.Element;
            if (localRendererElement != null)
            {
                Canvas.SetZIndex(localRendererElement, Canvas.GetZIndex(LocalVideo));
            }
            if (remoteRendererElement != null)
            {
                Canvas.SetZIndex(remoteRendererElement, Canvas.GetZIndex(RemoteVideo));
            }
#endif
            ...
            batch.Completed += (s, args) =>
            {
                if (collapsed == VoipVideoActiveState.Local)
                {
                    ...
                    if (localRendererElement != null)
                    {
                        localRendererElement.Visibility = Visibility.Collapsed;
                    }
#endif
                }
                else if (collapsed == VoipVideoActiveState.Remote)
                {
                    ...
                    if (remoteRendererElement != null)
                    {
                        remoteRendererElement.Visibility = Visibility.Collapsed;
                    }
#endif
                }
            ...
            // Background from small to big. Local functions, not shared instances - see the
            // comment in ShowHideCollapsedToMaximized (Uno keys RegisterAnimation by instance).
            Vector3KeyFrameAnimation MakeScale1()
            {
                var animation = compositor.CreateVector3KeyFrameAnimation();
                animation.InsertKeyFrame(show ? 0 : 1, new Vector3(ratio1, ratio1, 0));
            ...
            // Same seam as ShowHideCollapsedToMaximized above (CreatePathKeyFrameAnimation
            // throws on Uno/Skia) and same substitute: jump the clip geometry straight to
            // whichever end `show` lands on instead of animating the path itself.
            geometry1.SetPath(show
                ? new CompositionPath(CanvasGeometry.CreateRoundedRectangle(null, 0, 0, ActualSize.X, ActualSize.Y, 0, 0))
                : new CompositionPath(CanvasGeometry.CreateRoundedRectangle(null, side, part, ActualSize.X - side * 2, ActualSize.Y - part * 2, 8f * ratio2, 8f * ratio2)));
#else
            var path1 = compositor.CreatePathKeyFrameAnimation();
        ...
        private void ShowHideCollapsedToMinimized(bool show, UIElement element, Vector2 frame)
        {
            ...
            // Background from small to big. Local function, not a shared instance - see the
            // comment in ShowHideCollapsedToMaximized (Uno keys RegisterAnimation by instance).
            Vector3KeyFrameAnimation MakeScale1()
            {
                var animation = compositor.CreateVector3KeyFrameAnimation();
                animation.InsertKeyFrame(show ? 0 : 1, new Vector3(0, 0, 0));
                animation.InsertKeyFrame(show ? 1 : 0, new Vector3(ratio1, ratio1, 0));
                animation.Duration = duration;
                return animation;
            }
            ...
            {
                rendererVisual.StartAnimation("Scale", MakeScale1());
                rendererVisual.Offset = visual1.Offset;
                rendererVisual.Opacity = 1;
            }

            // `geometry.Path = path` no compila aqui: CompositionPath es el sustituto de
            // Telegram.Linux/Graphics/CompositionPath.cs (Uno solo dibuja su propio
            // SkiaGeometrySource2D), y la propiedad esta tipada sobre la clase de Uno. La
            // extension SetPath del mismo fichero es el equivalente.
            geometry1.SetPath(new CompositionPath(CanvasGeometry.CreateRoundedRectangle(null, side, part, ActualSize.X - side * 2, ActualSize.Y - part * 2, 8f * ratio2, 8f * ratio2)));
#else
...
#if LINUX
            // After the sinks: Dispose unsubscribes and joins the worker, and a worker still
            // converting a frame out of a sink that has just been released is the shape of crash
            // this arc is trying not to add.
        ...
        private void OnSizeChanged(object sender, SizeChangedEventArgs e)
        {
            ...
            if (_minimizedVideo != VoipVideoActiveState.None)
            {
...
#if LINUX
                var rendererElement = RendererFor(element)?.Element;
                var rendererVisual = rendererElement != null ? ElementComposition.GetElementVisual(rendererElement) : null;
                if (rendererVisual != null)
                {
                    rendererVisual.Clip = clip1;
                }
                ...
                {
                    rendererVisual.CenterPoint = visual1.CenterPoint;
                    rendererVisual.Scale = visual1.Scale;
                    rendererVisual.Offset = visual1.Offset;
                    rendererVisual.Opacity = 1;
                }
        ...
        private void AcceptVoice_Click(object sender, RoutedEventArgs e)
        {
            _call.Accept(false);
        }

        private void AcceptVideo_Click(object sender, RoutedEventArgs e)
        {
            _call.Accept(true);
        }
        ...
        private async void Screen_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                {
#if LINUX
                    // No general picker yet (see CaptureSessionService.Linux.cs remarks): the
                    // PipeWire portal source is the only one worth auto-selecting today, and
                    // ChooseAsync would just return null for it, so this goes straight to
                    // FindPipeWireSourceId instead of through ChooseAsync.
                    await Task.Yield();

                    var sourceId = CaptureSessionService.FindPipeWireSourceId();
                    if (sourceId == null)
                    {
                        Logger.Info("Screen sharing: no PipeWire source offered (not a Wayland session, or calls are unavailable)");
                        return;
                    }

                    _call.ShareScreen(sourceId);
#else
...
#if LINUX
                // ConnectedAnimationService.GetForCurrentView() throws NotImplementedException on
                // Uno. This one is called RAW -- no try, no guard -- from the encryption-emoji tap
                // DURING AN ACTIVE CALL, so it took the call page down at the worst possible
                // moment. Same API and same substitute as ChatView.AnimateEntrance, but NOT the
                // same shape: there the whole method WAS the animation, so an early return lost
                // only polish. Here the method's job is to SHOW the popup that explains what the
                // emoji means, and returning would trade a crash for a button that does nothing.
                //
                // So only the morph is dropped. The popup still opens and its title is still
                // styled below; what is lost is the emoji flying into the popup and back.
                // Emoji.Opacity is deliberately left alone: the only thing that set it to 0 was
                // the animation's completion, so without the animation there is nothing to undo.
                ConnectedAnimationService service = null;
