...
#if !LINUX
// Telegram/ViewModels/Stories is not compiled on Linux (8.513 lines of stories subsystem). The
// only thing this file wanted from it is the date-header branch below.
...
namespace Telegram.Views
{
    public sealed partial class ProfilePage : HostedPage, IProfileDelegate, INavigablePage
    {
        ...
        private void InitializeScrolling()
        {
            ...
            // Same two substitutions as ProfileHeader.InitializeScrolling, for the same measured
            // reasons (PORTING.md §6): "scrollViewer" is a Visual here because a
            // CompositionPropertySet never notifies when it is written, so an expression reading one
            // is evaluated once and frozen; and the expression lexer cannot start an identifier on
            // an underscore. Nothing else about these expressions changes.
            ...
            scrolling = ElementCompositionPreview.GetScrollingCarrier(properties);
#else
            const string ScrollY = "scrollViewer.Translation.Y";
            const string Set = "_";
#endif

            var translation = visual.Compositor.CreateExpressionAnimation(ViewModel.IsSavedMessages ? $"-{ScrollY}" :
                $"{Set}.ActualHeight > 16 ? {ScrollY} > -({Set}.ActualHeight + 8) ? 0 : -{ScrollY} - ({Set}.ActualHeight + 8) : -{ScrollY}");
            translation.SetReferenceParameter("scrollViewer", scrolling);
            translation.SetReferenceParameter(Set, ProfileHeader.Properties);

            var clip = visual.Compositor.CreateExpressionAnimation(
                $"-{ScrollY} + 4");
            clip.SetReferenceParameter("scrollViewer", scrolling);
        ...
        protected override void OnNavigatedTo(NavigationEventArgs e)
        {
            ...
            {
                // See InitializeScrolling above for why "scrollViewer" is a Visual on Linux.
                CompositionObject scrolling = properties;
                ...
                var expOut2 = $"clamp(1 - ((-({ScrollY} + 148) / 32) * 0.2), 0.8, 1)";
                var slideOut2 = properties.Compositor.CreateExpressionAnimation($"vector3({expOut2}, {expOut2}, 1)");
                slideOut2.SetReferenceParameter("scrollViewer", scrolling);

                var slideOut3 = properties.Compositor.CreateExpressionAnimation($"-clamp(((-({ScrollY} + 148) / 32) * 16), 0, 16)");
                slideOut3.SetReferenceParameter("scrollViewer", scrolling);
        ...
        private void ShowHideDateHeader(bool show, bool animate)
        {
            ...
            // thing that ever collapses DateHeader again: without it the floating date stays in the
            // tree at opacity 0, eating pointer events over the tab list. No Duration is set, which
            // in Uno means the animation is done on its first evaluation, so one frame is the wait.
            // See Telegram.Linux/Xaml/CompositionScopedBatchEx.cs.
            batch.EndWithCompleted(opacity.Duration, BatchCompleted);
#else
            batch.End();
#endif
        }
...
#if LINUX
            // Unconditional, and the `_backgroundUpdated` guard is exactly why. Upstream only
            // bothers to collapse BackgroundRoot once it has been Update()d, because a
            // ChatBackgroundPresenter that was never given a wallpaper paints nothing. The Skia
            // stand-in does not share that assumption: with no Background set,
            // ChatBackgroundRenderer falls back to _defaultColors -- {0xDBDDBB, 0x6BA587,
            // 0xD5D88D, 0x88B884}, Telegram's default freeform wallpaper -- so an un-updated
            // control paints a full-size green-to-yellow gradient. On a profile that is every
            // pixel of the card area behind the info rows, the tab strip and the tab body, and it
            // washes the whole screen out (the tab labels are drawn on it at 5% white).
            //
            // Only the chat-hosted tabs (Saved Messages, Polls -- IProfileChatPage) want a chat
            // background behind them, and the branch above turns it on for exactly those.
            else
            {
...
#if LINUX
            // CompositionGeometricClip does not override SetAnimatableProperty, so in Uno an
            // animation on "Offset.Y" lands in the generic property set of the clip and the clip's
            // own _offset field is never touched: it compiles, it does not throw, and the clip never
            // moves (PORTING.md §6). Unlike the rest of the header choreography this one cannot be
            // salvaged by carrying the position on a Visual, because the failure is on the writing
            // side, not the reading side. Driven from OnViewChanged instead — CompositionClip.Offset
            // is a normal setter and it does notify.
            _backgroundClip = clip;
            _backgroundClipHeaderHeight = (float)Math.Max(Header.ActualSize.Y, 48 + 10);
        ...
        private void BindToolTipToThumb(UIElement element)
        {
            ...
            if (element is not null)
            {
                ...
                // "this" is not in the vocabulary of Uno's expression lexer: it lexes as a plain
                // identifier and then fails to resolve — ArgumentException "Unrecognized identifier
                // 'this'" thrown from StartAnimation, i.e. from inside a visual state change of the
                // scrollbar. Here this.Target is the tooltip's own visual, which is in hand.
#if LINUX
                var animation = visual.Compositor.CreateExpressionAnimation("thumb.Offset.Y + (target.Size.Y - self.Size.Y) / 2");
                animation.SetReferenceParameter("self", visual);
#else
        ...
        {
#if LINUX
            // ViewChanged is the ONLY scroll event Uno raises: ViewChanging,
            // DirectManipulationStarted and DirectManipulationCompleted are all [NotImplemented] for
            // __SKIA__ and only log. So everything the header choreography needs is published from
            // here — first, because the whole collapsing header reads it.
            ElementCompositionPreview.UpdateScrollingPosition(ScrollingHost, ScrollingHost.VerticalOffset);
            UpdateBackgroundClip();

            // OnDirectManipulationStarted is what raised this flag upstream. An intermediate
            // ViewChanged is the same statement — the view is moving under someone's finger or
            // wheel — and it is the one Uno does deliver. Without it the page would keep deciding
            // it had never been scrolled and yank itself back to the content on every settle.
            //
            // One behaviour does change, and it changes for the better: upstream lowers the flag
            // again in OnDirectManipulationCompleted, so afterwards, settling at offset 0 re-runs
            // the ScrollToContent below and drags the user back down to the tab list. There is no
            // "manipulation completed" here to lower it on, so scrolling up to the very top stays
            // at the top. The opening position is unaffected: the flag still starts false, so the
            // first settle after navigation still runs ScrollToContent.
            if (e.IsIntermediate)
            {
                _hasBeenScrolled = true;
            }
            ...
            {
                // ItemsStackPanel.FirstVisibleIndex is [NotImplemented] but has a real body in Uno
                // (_layout?.FirstVisibleIndex ?? -1) and answers correctly. ItemsWrapGrid's getter
                // is a bare throw, so on Linux the media grid falls to the -1 branch, which is the
                // one that estimates the date from the scroll offset instead of from the first
                // visible item. That branch only exists for MediaDataSource, so the media tab keeps
                // its scrollbar tooltip and the other tabs lose the floating date header. Measured
                // on the deployed Uno.UI; see PORTING.md §6.
#if !LINUX
...
#if LINUX
            // Assigned, not animated, and the two reasons are worth keeping straight.
            //
            // The one that makes it necessary: this method is called from OnNavigatedTo when the
            // chat is Saved Messages, that is BEFORE the page is attached to a composition target,
            // and Compositor.RegisterAnimation drops a KeyFrameAnimation whose visual has no
            // CompositionTarget yet. The animation then never advances and the property keeps the
            // single value StartAnimation wrote, which is the keyframe at progress ZERO - here the
            // card still opaque and the menu button still invisible, i.e. exactly backwards. Same
            // trap as ChatView.UpdateSendButtons, where it emptied the right end of the composer.
            //
            // The one that makes it free: neither animation sets Duration, and in Uno an unset
            // Duration is TimeSpan.Zero, so KeyFrameEvaluator reports the final value on its first
            // evaluation and the "animation" is a jump anyway. Nothing is lost by jumping here.
            //
            // No StopAnimation needed first: on this platform this method never starts one, so
            // there is never a finished animation left owning Opacity (PORTING.md §6).
            cardBackground.Opacity = show ? 0 : 1;
        ...
        private async void LoadMore(ListViewBase scrollingHost)
        {
            ...
            uint loadedMore = 0;
#if LINUX
            // LastCacheIndex THROWS NotImplementedException on both ItemsStackPanel and ItemsWrapGrid
            // in Uno (measured on the deployed Uno.UI: the getter body is a throw, not a stub that
            // returns -1). LoadMore is called from OnViewChanged, OnSizeChanged, OnNavigated,
            // OnItemsSourceChanged and OnCollectionChanged, so leaving it would take the scroll
            // handler and a SizeChanged handler down with it.
            //
            // The question upstream asks is "is the last item already realized", i.e. is the tab
            // list scrolled to its end. Asked of the OUTER ScrollViewer instead, which is the one
            // that actually scrolls here: the tab list has no viewport of its own, it is stretched
            // to its whole content inside MediaFrame. One viewport of slack so the next page starts
            // loading before the user reaches the bottom, matching what a cache length would buy.
...
#if LINUX
            // The keyframes go in descending order of progress when show is false. That is safe:
            // Vector3KeyFrameAnimation and ScalarKeyFrameAnimation both hold their frames in a
            // SortedDictionary<float, ...>, so insertion order decides nothing and the final value
            // is the frame at progress 1 either way. (Worth stating: PORTING.md §6 lists this
            // ordering as a suspect for a different defect, and it is not one.)
            //
            // Completed, on the other hand, never fires - and this handler is the only thing that
            // ever collapses ManagePanel, which otherwise stays in the tree at opacity 0 swallowing
            // clicks along the bottom of the page.
            batch.EndWithCompleted(opacity1.Duration, BatchCompleted);
#else
            batch.End();
#endif
        }
