    ...
    public partial class DownloadFolderService : IDownloadFolderService
    {
        ...
        public async Task<StorageFolder> GetDefaultFolderAsync()
        {
            if (ApiInfo.HasKnownFolders)
            {
                ...
                {
#if LINUX
                    // Unreachable on this head and deliberately so: ApiInfo.HasKnownFolders is a
                    // compile-time false here, because downloads stay in the TDLib cache (see the
                    // note at the top of ApiInfo.cs). The branch only has to COMPILE, and
                    // KnownFolderId.DownloadsFolder does not exist on Uno.
                    //
                    // Do not "fix" this by reading XDG_DOWNLOAD_DIR: it is not an environment
                    // variable on a normal desktop (it lives in ~/.config/user-dirs.dirs, read via
                    // xdg-user-dir), and ~/Downloads is not the folder name under every locale.
                    // Giving this head a real download folder means flipping HasDownloadFolder and
                    // reading user-dirs.dirs, which is a feature, not a merge fix.
                    return null;
#else
