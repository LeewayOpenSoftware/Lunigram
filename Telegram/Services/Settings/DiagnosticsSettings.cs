...
namespace Telegram.Services.Settings
{
    public partial class DiagnosticsSettings : SettingsServiceBase
    {
        ...
        {
            // Upstream defaults this to DEBUG builds only, for taking clean screenshots without
            // leaking a real number. The Linux port's Debug build is the user's daily driver, not
            // a screenshots build, so that default would hide their real phone number every day.
            get => _hidePhoneNumber ??= GetValueOrDefault("HidePhoneNumber",
#if LINUX
                false);
#else
                Constants.DEBUG);
