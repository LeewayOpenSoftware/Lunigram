...
#else
                    // Not RootWindow: the Linux head has its own RootPage (Telegram.Linux/Hubs),
                    // which upstream's RootPage -> RootWindow rename does not touch.
                    if (window.Content is RootPage root && root.NavigationService?.Session == this)
#endif
                    {
