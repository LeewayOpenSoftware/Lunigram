...
namespace Telegram.Services
{
    ...
    public static partial class AppSettings
    {
        ...
        {
#if LINUX
            // On Windows the tray icon is a debugging nuisance (it comes with a second process),
            // hence RELEASE. On Linux it lives in this process and it is the only way to reach the
            // app once its window is closed, so it is on from the start; the toggle that changes
            // it lives in Settings > Advanced, which is not in the subset yet.
            get => _isTrayVisible ??= _local.GetValueOrDefault("IsTrayVisible", true);
#else
        ...
        {
            // Upstream's own default is only true for a brand new install: any Windows client that
            // updates gets it turned off in Initialize() below. So the tab strip across the top is
            // what Unigram actually looks like, and the rail down the left is what a first run
            // happens to inherit. This port has only ever had first runs.
            get => _useLeftTabsForChats ??= _local.GetValueOrDefault("IsLeftTabsEnabled",
#if LINUX
                false);
#else
                true);
