...
namespace Telegram.Services.Calls
{
    ...
    public partial class VoipGroupCall : VoipCallBase
    {
            ...
            : base(clientService, settingsService, aggregator)
        {
            ...
            {
                IsConference = false,
                VideoContentType = VoipVideoContentType.Generic,
            ...
            lock (_managerLock)
            {
                ...
                _manager?.EmitJoinPayload(async (ssrc, payload) =>
                {
                    ...
                    if (response is Text json && _manager != null)
                    {
                        bool broadcast;
#if LINUX
                        // Uno's Windows.Data.Json.JsonObject has no TryParse, and the head's
                        // GetNamedBoolean helper (Extensions.Linux.cs:180) is written against
                        // System.Text.Json.Nodes.JsonObject. Parse throws instead of returning
                        // false, so the try/catch is the TryParse contract, not defensiveness.
                        try
                        {
                            broadcast = System.Text.Json.Nodes.JsonNode.Parse(json.TextValue) is System.Text.Json.Nodes.JsonObject data
                                && data.GetNamedBoolean("stream", false);
                        }
        ...
        public VoipCaptureBase VideoCapturer => _capturer;

        public Func<string, VoipCaptureBase> VideoCaptureFactory { get; set; }

        private VoipVideoOutputSink _localPreviewSink;

        public void SetLocalVideoOutput(VoipVideoOutputSink sink)
        {
            lock (_managerLock)
            {
                _localPreviewSink = sink;
                _capturer?.SetOutput(sink);
            }
        }

        public void SetVideoCaptureSource(VoipCaptureBase source)
        {
            lock (_managerLock)
            {
                if (_manager == null)
                ...
                {
                    _capturer = source;
                    _capturer.SetOutput(_localPreviewSink);
                    _manager.SetVideoCapture(_capturer);
                }
            }

            RaisePropertyChanged(nameof(IsVideoEnabled));
        ...
        public void ToggleCapturing()
        {
            if (IsVideoEnabled)
            {
                SetVideoCaptureSource(null);
            }
            else
            {
                var source = VideoCaptureFactory != null
                    ? VideoCaptureFactory(_videoInputId)
                    : new VoipVideoCapture(_videoInputId);
                SetVideoCaptureSource(source);
            }
        }
        ...
        public override string VideoInputId
        {
            ...
            set
            {
                ...
                (_capturer as VoipVideoCapture)?.SwitchToDevice(_videoInputId = value);
            }
        }
            ...
            lock (_managerLock)
            {
                switch (e.DeviceClass)
                {
                    case MediaDeviceClass.VideoInput:
                        (_capturer as VoipVideoCapture)?.SwitchToDevice(_videoInputId = e.DeviceId);
                        break;
