...
namespace Telegram.Services.Calls
{
    public partial class VoipCall : VoipCallBase
    {
        ...
        {
            ShareScreenCore(new VoipScreenCapture(item));
        }

#if LINUX
        /// <summary>
        /// The Linux twin of <see cref="ShareScreen(GraphicsCaptureItem)"/>: there is no
        /// <see cref="GraphicsCaptureItem"/> here, so the source is addressed by the id
        /// <see cref="Services.CaptureSessionService"/> enumerates (see its remarks for why
        /// that is, today, only ever the well-known PipeWire portal id).
        /// </summary>
        public void ShareScreen(string sourceId)
        {
            ShareScreenCore(new VoipScreenCapture(sourceId));
        }
#endif

        private void ShareScreenCore(VoipScreenCapture screen)
        {
            lock (_managerLock)
            {
        ...
        private void TransitionToReady(CallStateReady ready)
        {
            ...
            {
                Version = SelectBestProtocolVersion(ready.Protocol.LibraryVersions),
        ...
        private static string SelectBestProtocolVersion(IReadOnlyList<string> versions)
        {
            ...
            foreach (var preferred in new[] { "13.0.0", "12.0.0", "2.7.7" })
            {
                for (int i = 0; i < versions.Count; i++)
                {
                    if (string.Equals(versions[i], preferred, StringComparison.OrdinalIgnoreCase))
                    {
                        return preferred;
                    }
                }
        ...
        private void CreateWindow(bool newOutgoingCall)
        {
            ...
            Logger.Info($"Waiting for window creation (IsOutgoing={newOutgoingCall}, Thread={Environment.CurrentManagedThreadId})");
            _ = service.OpenAsync(options).ContinueWith(task =>
            {
                if (task.IsFaulted)
                {
                    Logger.Error($"CreateWindow OpenAsync FAULTED: {task.Exception}");
                }
                else if (task.IsCanceled)
                {
                    Logger.Warning("CreateWindow OpenAsync CANCELLED");
                }
                else if (task.Result == null)
                {
                    Logger.Warning("CreateWindow OpenAsync returned NULL WindowContext");
                }
                else
                {
                    Logger.Info($"CreateWindow OpenAsync SUCCEEDED (WindowId={task.Result.Id})");
                }
        ...
        private void DisposeImpl()
        {
            ...
            // _manager is already null (above, inside the lock), so every other lock-guarded
            // member (SetLocalVideoOutput/SetRemoteVideoOutput/ReceiveSignalingData/...) now
            // no-ops instead of touching this instance -- calling Stop/Dispose out here cannot
            // race them. The point of moving these out of the lock: unigram_voip_manager_stop/
            // _release (native tgcalls teardown) can block for several seconds finishing ICE/
            // DTLS shutdown -- known fragile territory (see SelectBestProtocolVersion's abort
            // hardening for the same hangup path). Holding _managerLock for that whole time
            // blocked VoipWindow.OnUnloaded's SetLocalVideoOutput/SetRemoteVideoOutput calls
            // (fired on the UI thread the instant PresentContent(null) dismisses the call
            // window), which in turn blocked the UI thread itself -- observed as the whole app
            // going black for as long as native teardown took (~15s) before RootPage's restored
            // content could repaint. Only the lock's hold time needed narrowing; nothing here
            // changes what gets torn down or in what order relative to DisposeCamera() above.
            if (manager != null)
            {
                ...
                manager.SetSignalingDataEmitted(null);
                manager.Dispose();
            }
        }
