...
namespace Telegram.Services
{
    ...
    // Linux: this class is shared, not reimplemented. Its playlist logic (MoveNext/MovePrevious per
    // message kind, the repeat modes, the PlaybackPreviousState stack that gives the music back
    // after a voice note) has nothing Windows about it; what did was AsyncMediaPlayer (libvlc, now
    // libmpv in Telegram.Linux/Native/Media/), SystemMediaTransportControls (declared for Uno in
    // Telegram.Linux/Xaml/, MPRIS2 behind it) and Attach/Detach(SwapChainPanel), which Uno declares
    // and does not implement. See unigram-linux/fase1/plan-media.md 5.1.
    public partial class PlaybackService : IPlaybackService
    {
        ...
        private void RunOnTransport(Action action)
        {
            ...
            {
                queue.TryEnqueue(new Microsoft.UI.Dispatching.DispatcherQueueHandler(action));
            }
        }
        ...
        private void SeekImpl(AsyncMediaPlayer player, TimeSpan span)
        {
            ...
            // Not needed with libmpv, and measured rather than assumed: a backwards seek in an
            // .ogg/opus voice note takes 4-7 ms and time-pos lands exactly on the target
            // (unigram-linux/fase1/plan-media.md 2.1). Stopping and restarting here would also
            // undo the pending-position handling of the Linux player.
