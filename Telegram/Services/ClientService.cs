...
namespace Telegram.Services
{
    ...
    public partial interface ICacheService
    {
        ...
        // Same passthrough as Notifications: 12.10 moved the recent-emoji list onto the
        // per-account settings, and callers that only hold a client service need a route to it.
        Settings.RecentEmojiSettings RecentEmoji { get; }
    ...
    public partial class ClientService : IClientService, ClientResultHandler
    {
        ...
        private void Initialize(bool online = true)
        {
            ...
            Task.Factory.StartNew(() =>
            {
                ...
                // Kept as a local (upstream sends it inline): the LINUX branch below needs the
                // parameters again to attach a result callback.
                var parameters = new SetTdlibParameters(
...
#if LINUX
                // Constants.Secret.cs may still hold the placeholders (ApiId = 0): TDLib rejects the
                // parameters and never leaves WaitTdlibParameters, so OnStart would wait forever on
                // a blank window. Surface the login page anyway; every later request reports the error.
                Send(parameters, result =>
                {
                    if (result is Error error)
                    {
                        Logger.Error($"setTdlibParameters failed: {error.Code} {error.Message}");
                        _authorizationState ??= new AuthorizationStateWaitPhoneNumber();
                        _authorizationStateTask.TrySetResult(true);
                    }
                });
#else
                _client.Send(parameters);
#endif
...
#if LINUX
            // Porting aid: UNIGRAM_TD_REQUESTS=1 turns the td_requests tag on for THIS RUN ONLY.
            // The tag makes TDLib dump every request and response, contents included, to
            // tdlib_log.txt, so it must never be left on by accident. Setting it through the
            // persistent Diagnostics container does exactly that, which is why this exists.
            if (Environment.GetEnvironmentVariable("UNIGRAM_TD_REQUESTS") == "1")
            {
                Client.Execute(new SetLogTagVerbosityLevel("td_requests", 1));
            }
#endif
        }
        ...
        public async Task<Object> SendPaymentAsync(long starCount, Function function)
        {
            if (OwnedStarCount.StarCount < starCount)
            {
                ...
                {
#if LINUX
                    // Td/Api/ErrorStarsNeeded.cs is out of the subset (its consumers are the Stars popups).
                    return new Error(400, "BALANCE_TOO_LOW");
#else
        ...
        public Settings.RecentEmojiSettings RecentEmoji => _settings.RecentEmoji;
