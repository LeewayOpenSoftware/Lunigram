...
namespace Telegram.Controls.Gallery
{
    ...
    //, IHandle<UpdateMessageContent>
    {
        ...
        {
            AutomationProperties.SetName(BackButton, Strings.AccDescrGoBack);
            ToolTipService.SetToolTip(BackButton, Strings.AccDescrGoBack);

            AutomationProperties.SetName(ZoomOut, Strings.MediaZoomOut);
            ToolTipService.SetToolTip(ZoomOut, Strings.MediaZoomOut);

            AutomationProperties.SetName(ZoomIn, Strings.MediaZoomIn);
            ToolTipService.SetToolTip(ZoomIn, Strings.MediaZoomIn);

            AutomationProperties.SetName(Rotate, Strings.AccDescrRotate);
            ToolTipService.SetToolTip(Rotate, Strings.AccDescrRotate);
        }
        ...
        private void ShowHideTransport(bool show)
        {
...
#if !LINUX
            // Window.Current.CoreWindow es null en Uno (PORTING.md 6): esto es un NRE en cada
            // aparición/desaparición de la barra inferior. Lo que se pierde es ocultar el puntero
            // del ratón mientras la galería está quieta.
            if (show)
            {
            ...
            // One animation instance per visual: Uno's Compositor.RegisterAnimation indexes its
            // dictionary by the animation object, so starting the same instance on the second
            // visual throws "An item with the same key has already been added" - and here it threw
            // from the inactivity timer's Tick, which is what left the bottom bar of the gallery
            // stuck. See PORTING.md 6; on Windows the result is identical.
            ScalarKeyFrameAnimation Opacity()
            {
                var animation = parent.Compositor.CreateScalarKeyFrameAnimation();
...
#if LINUX
            // CompositionScopedBatch.Completed no se dispara en Uno (PORTING.md 6). Sin el sustituto,
            // la barra inferior se queda para siempre sin recibir toques después de ocultarse una vez.
            batch.EndWithCompleted(opacity.Duration, () => BottomPanel.IsHitTestVisible = !_transportCollapsed);
#else
            batch.End();
#endif
        }
        ...
        {
#if !LINUX
            // ConnectedAnimationService.GetForCurrentView() lanza en Uno (PORTING.md 6, mismo caso
            // que ChatView.AnimateEntrance): aquí tumbaría la apertura de la galería entera. Lo que
            // se pierde es la transición en la que la foto del chat "vuela" a pantalla completa.
...
#if LINUX
                // ApplicationView is not implemented by Uno, so full screen goes through the
                // AppWindow presenter instead (see SetFullScreenMode below). There is no
                // VisibleBoundsChanged to subscribe to either: the layout half is driven by hand
                // from the two places that change the mode.
                _wasFullScreen = IsFullScreenMode();
                _lastFullScreen = !_wasFullScreen;
                UpdateFullScreen(_wasFullScreen);

                if (AppSettings.FullScreenGallery && !_wasFullScreen && SetFullScreenMode(true))
                {
...
#if LINUX
                    // The PrepareNext above ran with an EMPTY collection - the items only exist
                    // once this await has finished - so it returned at its own
                    // "index < 0 || Items.Empty()" guard without setting a single image and without
                    // setting HasPrevious/HasNext. The CollectionChanged subscription does not save
                    // it either: it was made on the collection the view model had *then*. Measured
                    // on 2026-08-24: the gallery opened with its three elements at their empty size
                    // of 48x100, and the carousel answered "prev False, next False" to every swipe
                    // while the bottom bar read "681 of 695".
                    PrepareNext(CarouselDirection.None, true, false, player);
#endif
                }
            });
...
#if LINUX
            // Loaded NEVER fires for this window on Uno, and everything that makes the gallery a
            // gallery hangs off it: measured on 2026-08-24 with a probe inside PrepareNext, which
            // was not called once in a whole session - the window showed its dark layer, its title
            // bar and its bottom bar (those come from Load(parameter) + Bindings.Update()), the
            // three GalleryContent elements stayed at their empty 48x100, no picture was ever set
            // into them, and the carousel answered "prev False, next False" to every swipe while
            // the bottom bar read "681 of 695".
            // LayoutUpdated does arrive - it is the same escape hatch this port already uses for
            // ItemsPanelRoot (PORTING.md 6) - and the handler unsubscribes itself from Loaded on
            // its way out, so it cannot run twice.
            EventHandler<object> once = null;
            once = (s, args) =>
            {
                LayoutUpdated -= once;

                // Uno takes the window out of the tree on the way in (the popup reparents its
                // child), which runs OnUnloaded before anything else; the flag would make the
                // handler return at its first line.
                _unloaded = false;
                handler(this, null);
            };
        ...
        {
            BackButton.Glyph = "\uE72B";
            BackButton.Margin = new Thickness(0, TitleBarInset, 0, 0);
            BackButton.HorizontalAlignment = HorizontalAlignment.Left;
        }

        /// <summary>
        /// How far the gallery's chrome reaches above the top of its own window.
        ///
        /// On Windows the app extends its content into the title bar and draws a caption of its
        /// own, so the back button, the backdrop and the two hit-test regions are pulled up by the
        /// forty pixels that caption occupies. The port sets ExtendViewIntoTitleBar to false - the
        /// caption is Windows-only code, and dropping the frame would leave nothing to drag or
        /// resize by - so there is no strip up there to reach into: forty pixels above the top of
        /// the window is off the window, which is where the back button was going.
        /// </summary>
        private const double TitleBarInset =
#if LINUX
            0;
#else
            -40;
        ...
        /// <summary>
        /// The same forty pixels, for the four elements that carry them in the markup. They are
        /// set here rather than in the XAML because the file is shared with the Windows build,
        /// where the offset has to stay.
        /// </summary>
        private void ApplyTitleBarInset()
        {
            var inset = new Thickness(0, TitleBarInset, 0, 0);

            Layer.Margin = inset;
            LayerFullScreen.Margin = inset;
            SafeArea.Margin = inset;
            TitleBar.Margin = inset;
        }

        private void OnVisibleBoundsChanged(ApplicationView sender, object args)
        {
            UpdateFullScreen(sender.IsFullScreenMode || ApiInfo.IsXbox);
        }

#if LINUX
        // Uno's own AppWindow presenter is what replaces ApplicationView.TryEnterFullScreenMode
        // here: SetPresenter(FullScreen) lands on X11WindowWrapper.ApplyFullScreenPresenter ->
        // SetFullScreenMode(true), which sets _NET_WM_STATE_FULLSCREEN on the window, and the
        // Overlapped one lands on ApplyOverlappedPresenter, which clears it (both read off the
        // deployed Uno.UI.Runtime.Skia.X11.dll with unigram-linux/spikes/UnoApiProbe).
        //
        // Going back is done with the SAVED presenter object rather than with
        // AppWindowPresenterKind.Overlapped, because that kind builds a brand new
        // OverlappedPresenter and would throw away the minimum window size WindowContext sets at
        // startup. And CompactOverlay is the one kind Uno's SetPresenter throws on - nothing here
        // ever asks for it, the mini player is an accepted loss under Wayland.
        private AppWindowPresenter _presenter;

        private AppWindow GetAppWindow()
        {
            var window = WindowContext.ForXamlRoot(this);
            return window != null ? ((Window)window).AppWindow : null;
        }
        ...
        private bool IsFullScreenMode()
        {
            try
            {
                return GetAppWindow()?.Presenter is FullScreenPresenter;
            }
            catch (Exception ex)
            {
        ...
        private bool SetFullScreenMode(bool fullScreen)
        {
            try
            {
                ...
                if (fullScreen)
                {
                    _presenter ??= appWindow.Presenter;
            ...
            {
                // A gallery that cannot go full screen is still a gallery.
                Logger.Error(ex.ToString());
                return false;
            }
        }
...
#if LINUX
            // Same shape as the Windows path below: the gallery only undoes a full screen it put
            // the window into itself, and Escape from full screen is a step back to the windowed
            // gallery rather than a close. Leaving this out would strand the MAIN window full
            // screen, with no title bar, after the gallery is gone.
            if (_lastFullScreen && !_wasFullScreen)
            {
                if (SetFullScreenMode(false))
                {
...
#if LINUX
            // CompositionScopedBatch.Completed no se dispara en Uno (PORTING.md 6): sin el sustituto
            // la galería se desvanece pero NUNCA se cierra, y se queda tapando el chat para siempre.
            batch.EndWithCompleted(TimeSpan.FromSeconds(0.167), () => OnClosingCompleted(null, null));
#else
...
#if LINUX
            // ConnectedAnimationService.GetForCurrentView() lanza en Uno, y esta línea corre en cada
            // apertura y en cada cierre. La curva por defecto de WinUI para las conectadas es una
            // cubic-bezier suave; se pone a mano la equivalente.
            scalar.InsertKeyFrame(1, to, _layer.Compositor.CreateCubicBezierEasingFunction(new Vector2(0.1f, 0.9f), new Vector2(0.2f, 1f)));
#else
...
#if LINUX
            Telegram.Common.GestureProbe.Log($"gallery PrepareNext({direction}, initialize {initialize}): "
                + $"index {index}, items {viewModel.Items.Count}");
#endif
...
#if LINUX
            Telegram.Common.GestureProbe.Log($"gallery prepared: item {index + 1} of {viewModel.Items.Count}, "
                + $"prev {LayoutRoot.HasPrevious}, next {LayoutRoot.HasNext}");
#endif

#if LINUX
            // UIViewSettings.GetForCurrentView() no existe en Uno. Las flechas de anterior/siguiente
            // se dejan puestas: en la Surface se usa el dedo, pero también hay ratón, y una flecha
            // que no molesta es mejor que una excepción en cada cambio de foto.
            if (true)
            {
#else
        ...
        {
#if LINUX
            var fullScreen = !_lastFullScreen;

            if (SetFullScreenMode(fullScreen))
            {
        ...
        {
            AutomationProperties.SetName(Recognize, Strings.AccDescrScanText);
            ToolTipService.SetToolTip(Recognize, Strings.AccDescrScanText);
