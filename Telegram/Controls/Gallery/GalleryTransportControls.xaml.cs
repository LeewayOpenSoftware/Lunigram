...
namespace Telegram.Controls.Gallery
{
    public sealed partial class GalleryTransportControls : UserControl
    {
        ...
        {
            AutomationProperties.SetName(VolumeSlider, Strings.AccDescrVolume);

            AutomationProperties.SetName(SkipBackButton, Strings.AccDescrMiniPlayer);
            ToolTipService.SetToolTip(SkipBackButton, Strings.AccDescrMiniPlayer);

            AutomationProperties.SetName(SkipForwardButton, Strings.AccDescrMiniPlayer);
            ToolTipService.SetToolTip(SkipForwardButton, Strings.AccDescrMiniPlayer);

            AutomationProperties.SetName(FullScreenButton, Strings.AccDescrFullScreen);
            ToolTipService.SetToolTip(FullScreenButton, Strings.AccDescrFullScreen);

            AutomationProperties.SetName(CompactButton, Strings.AccDescrMiniPlayer);
            ToolTipService.SetToolTip(CompactButton, Strings.AccDescrMiniPlayer);

            AutomationProperties.SetName(Slider, Strings.AccDescrSeek);
        }
...
#if LINUX
            // Common/ImageHelper.cs is excluded from the Linux subset (Win2D, BufferSurface,
            // BitmapDecoder - all of it for the send path), and FitRatio is the one line of it
            // this file needs. Same arithmetic, inlined, rather than dragging the file in.
            _storyboardScale = Math.Min(144d / width, 144d / height);
#else
        ...
        private void Volume_Click(object sender, RoutedEventArgs e)
        {
            ...
            // Upstream persists only the mute flag here. Keep our VolumeLevel write too: when the
            // block above rescues volume 0 -> 100, not storing it means the rescue is forgotten.
            AppSettings.VolumeLevel = volume / 100d;
            AppSettings.VolumeMuted = muted;

            // The slider is 0..1, like every other use of it in this file; `volume` is 0..100, and
            // passing it raw left the bar full and its thumb outside its own 72 px for any volume
            // above zero. Seen in the app the moment the mute button was pressed twice.
            VolumeSlider.UpdateValue(muted ? 0 : volume / 100d, 1, false);
