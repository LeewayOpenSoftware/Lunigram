...
#if LINUX
                // One player, not three: WebVideoPlayer and its ChromiumWebPresenter are WebView2,
                // which Uno does not have, and HLS with it. LinuxVideoPlayer covers everything
                // else -- mpv for the sound and the clock, our own decoder for the pixels.
                if (Video is not LinuxVideoPlayer)
                {
                    Video = new LinuxVideoPlayer();
                }
#else
...
#if LINUX
        // OCR of the picture: it is Telegram.Native.AI (RecognizedText, RecognizedTextSelection...),
        // which has no Linux side yet, and ImageTextSelection is an inert stub here
        // (Telegram.Linux/Xaml/ImageTextSelectionStub.cs). The button that calls this is x:Load'ed
        // on GalleryMedia.CanRecognizeText, so it is not even created unless something says the
