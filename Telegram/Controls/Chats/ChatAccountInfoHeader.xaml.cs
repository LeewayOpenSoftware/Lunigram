        ...
        public void UpdateUser(IClientService clientService, Chat chat, User user, UserFullInfo fullInfo)
        {
            ...
            if (chat.ActionBar != null && user.EmojiStatus != null)
            {
                ...
                if (markdown.Entities.Count == 2)
                {
                    ...
                    {
#if LINUX
                        // The link's destination is PromoPopup, which is not in the subset: it asks
                        // for StickersPopup and its PromoViewModel asks for FeaturesPopup, i.e. the
                        // whole Premium subsystem, which is a batch of its own. The sentence still
                        // reads -- it is the explanation of why this account only takes messages
                        // from Premium users -- so the words stay; what goes is the link, because a
                        // link that opens nothing is worse than plain text.
