...
namespace Telegram.Controls.Chats
{
    ...
    public partial class ChatListViewAutomationPeer : ListViewItemAutomationPeer
    {
        ...
        {
            if (_owner.ContentRoot() is MessageSelector selector)
            {
            ...
            else if (_owner.ContentRoot() is MessageService service)
            {
                return AutomationProperties.GetName(service);
            }
            else if (_owner.ContentRoot() is StackPanel panel && panel.Children.Count > 0)
            {
