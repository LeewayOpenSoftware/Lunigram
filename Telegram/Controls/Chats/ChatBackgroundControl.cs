...
{
#if LINUX
    // A Grid, not a templated control: the Style that carries the template is win: only (it hosts
    // two AnimatedImage for gift patterns and a ChatBackgroundPresenter, all composition), and on
    // this head the presenter is replaced by ChatBackgroundCanvas -- a single Skia-painted child,
    // see Telegram.Linux/Xaml/ChatBackgroundCanvas.cs. Everything below the template is shared.
...
#if LINUX
        private readonly ChatBackgroundCanvas Presenter;
#else
...
#if LINUX
            Presenter = new ChatBackgroundCanvas();
...
#if LINUX
            // No thumbnail path and no ChatTheme gift patterns here: the canvas draws the fill and
            // the .tgv, which is what a chat background is unless a gift theme is applied.
            Presenter.UpdateSource(_clientService, background);
#else
...
#if !LINUX
    // ChatBackgroundPreview and ChatBackgroundFreeform paint their gradient through
    // WriteableBitmap.Buffer, an IBufferByteAccess QI that Uno does not answer (the extension is
    // itself #if !LINUX in Telegram/Common/Extensions.cs), so neither compiles here. The same
    // algorithm -- the eight anchors, the swirl and the phase walk -- lives in
    // Telegram.Linux/Graphics/ChatBackgroundRenderer.cs, drawn with Skia; the two consumers of
    // these types (the background pickers and TdExtensions.ToBrush) are out of the Phase 1 subset
    // or already degraded with an #if.
