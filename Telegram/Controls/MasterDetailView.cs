...
namespace Telegram.Controls
{
    ...
    public sealed partial class MasterDetailView : ContentControl, INotifyPropertyChanged, IDisposable
    {
        ...
        public void Dispose()
        {
            ...
            AppearanceSettings.TouchModeChanged -= OnTouchModeChanged;
        ...
        public void ShowHideBackground(BackgroundKind show, bool animate)
        {
...
#if LINUX
            // The animated path settles every Visibility from CompositionScopedBatch.Completed, and
            // that event never fires in Uno (PORTING.md section 6). Left as is, the parts that
            // should end up Collapsed stay Visible with Opacity 0 -- and an element at Opacity 0
            // still hit-tests, so MaterialPart would sit invisibly over the chat and eat pointer
            // input. Until there is a substitute for the batch, the crossfade is skipped and the
            // final state is applied directly, which is what the non-animated branch already does.
            animate = false;
#endif
            ...
            if (animate)
            {
                ...
                // One instance per target. Uno's Compositor keeps running KeyFrameAnimation
                // instances in a dictionary keyed by the animation instance (RegisterAnimation).
                // Concurrent StartAnimation calls on distinct visuals with the same instance throw
                // ArgumentException and freeze the dispatcher.
                ScalarKeyFrameAnimation CreateFadeOut()
                {
                ...
                ScalarKeyFrameAnimation CreateFadeIn()
                {
                    var animation = visual.Compositor.CreateScalarKeyFrameAnimation();
                ...
                {
                    visual.StartAnimation("Opacity", CreateFadeIn());

                    if (type == BackgroundKind.None)
                    {
                        border.StartAnimation("Opacity", CreateFadeIn());
                    }
                    else if (type == BackgroundKind.Material)
                    {
                        material.StartAnimation("Opacity", CreateFadeOut());
                        bread.StartAnimation("Opacity", CreateFadeOut());
                        button.StartAnimation("Opacity", CreateFadeOut());
                    }
                }
                else if (show == BackgroundKind.Material)
                {
                    material.StartAnimation("Opacity", CreateFadeIn());
                    bread.StartAnimation("Opacity", CreateFadeIn());
                    button.StartAnimation("Opacity", CreateFadeIn());
                    ...
                    {
                        visual.StartAnimation("Opacity", CreateFadeOut());
                    }
                }
                else if (show == BackgroundKind.None)
                {
                    border.StartAnimation("Opacity", CreateFadeOut());
        ...
        protected override void OnApplyTemplate()
        {
            ...
            AdaptivePanel.ForceMinimal = AppSettings.Appearance.TouchMode;
        ...
        public void ShowHideBanner(bool show)
        {
...
#if LINUX
                // On Uno an animation that has run out still owns the property it drove, so these
                // three writes are ignored unless the animation is stopped first. Measured on the
                // chat header, which this same shape left stuck 40 px above the chat for the rest
                // of the session (see ChatView.CompleteBannerAnimation); DetailRoot and
                // MasterFrame take the same path in the two states where they are the ones that
                // move -- a blank detail, and the narrow window.
                banner.StopAnimation("Translation.Y");
                detail.StopAnimation("Translation.Y");
                master.StopAnimation("Translation.Y");
#endif
            ...
            // One animation instance per visual: Uno's Compositor.RegisterAnimation keys its
            // dictionary on the animation object, so starting the same one on a second visual
            // throws (PORTING.md 6). Up to four visuals move with this banner.
            ScalarKeyFrameAnimation Translate(float from)
            {
                var animation = banner.Compositor.CreateScalarKeyFrameAnimation();
                animation.InsertKeyFrame(show ? 0 : 1, from);
            ...
            {
#if LINUX
                var minimal = Translate(-48);
#else
                translate.InsertKeyFrame(show ? 0 : 1, -48);
                var minimal = translate;
#endif

                MasterFrame.Margin = new Thickness(0, 0, 0, -48);
                master.StartAnimation("Translation.Y", minimal);
            }
            else
            {
                var detailVisual = ElementComposition.GetElementVisual(DetailPresenter);
                ...
                if (DetailFrame.Content is IChatPage chatPage)
                {
                    chatPage.StartBannerAnimation(Translate(-40));
                }
                else
                {
                    DetailRoot.Margin = new Thickness(0, 0, 0, -40);
                    detail.StartAnimation("Translation.Y", Translate(-40));
                }
            }

#if LINUX
            // CompositionScopedBatch.Completed never fires on Uno (PORTING.md 6), and this handler
            // is where the banner's scaffolding comes down: without it DetailRoot and MasterFrame
            // keep the -40/-48 margin and the Translation the animation borrowed, so the FIRST
            // voice note ever played would shift the whole main page for good, and the banner would
            // never collapse again. The path only became reachable in phase 5, when the shared
            // PlaybackService was turned on: until then CurrentItem was always null.
            batch.EndWithCompleted(translate.Duration, BatchCompleted);
#else
            batch.End();
#endif
        }
        ...
        /// <summary>
        /// Touch mode toggled from the settings page. The panel does the rest: setting
        /// <see cref="MasterDetailPanel.ForceMinimal"/> invalidates its measure, the next arrange
        /// publishes the new <see cref="MasterDetailState"/>, and <see cref="OnViewStateChanged()"/>
        /// runs the same visual state change a window resize would have run.
        ///
        /// The event is static (there is one settings tree, and this view is created per session),
        /// so the subscription is dropped in <see cref="Dispose"/> and re-made in
        /// <see cref="OnApplyTemplate"/>, never left behind on a recycled control.
        /// </summary>
        private void OnTouchModeChanged(object sender, EventArgs e)
        {
