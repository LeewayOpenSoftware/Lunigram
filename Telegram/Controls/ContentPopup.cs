...
namespace Telegram.Controls
{
    ...
    public partial class ContentPopup : ContentDialogEx
    {
        ...
        public ContentPopup()
        {
...
#if LINUX
            // u-wheel-scroll-dead: a popup is hosted in the XamlRoot's popup layer, a different
            // branch from the window content, so the wheel handler at the window root is not on
            // its route. Every popup in the app comes through here, so one line covers all of them.
            Telegram.Views.WheelScroll.Attach(this);
#endif
            ...
            Closing += OnClosing;
#if LINUX
            // Second door for _closingTask. On Windows the task is completed by Test(), which only
            // runs when LayoutRoot.IsHitTestVisible flips to false, and the ONLY thing that sets it
            // is an ObjectAnimationUsingKeyFrames inside <VisualTransition To="DialogHidden"> in
            // Themes/Generic.xaml. Uno does not run a VisualTransition storyboard, the DialogHidden
            // state itself is empty, so on Linux the flip never happens: ShowQueuedAsync awaits
            // forever, the confirmed action never runs, and because _currentDialogShowRequest is
            // only cleared after that await, every later dialog in the session is refused as busy.
            // Closed always fires, so completing from there costs nothing and unblocks every
            // confirmation in the app. See Telegram.Linux/PORTING.md §6.
            Closed += OnClosedLinux;
#endif
...
#if LINUX
        private void OnClosedLinux(ContentDialog sender, ContentDialogClosedEventArgs args)
        {
            // Test() is a TrySetResult, so whichever door gets there first wins and the other
            // one is a no-op. _closingResult has already been set by OnClosing.
            Test();
        }
#endif
        ...
        private void OnSizeChanged(object sender, SizeChangedEventArgs e)
        {
            ...
            // Uno registers a running animation by instance (Compositor.RegisterAnimation), so the
            // same object cannot drive two visuals at once: the border gets its own copy.
            var scale2 = compositor.CreateScalarKeyFrameAnimation();
            scale2.InsertKeyFrame(0, prev.Y / next.Y);
            scale2.InsertKeyFrame(1, 1);
            ...
            border.StartAnimation("Scale.Y", scale2);
            redirect.StartAnimation("Offset.Y", offset);
            clip.StartAnimation("BottomInset", inset);

#if LINUX
            // The clip is the grow animation, not the layout. A dialog that gets taller as its
            // content arrives is drawn at the new height behind an inset that closes from the
            // difference down to zero, so the old height appears to stretch into the new one.
            // Uno holds the first key frame of an animation on a CompositionClip property - the
            // same thing that kept the chat folder strip four pixels tall (b2feb38) - so the inset
            // stays at the difference and the dialog keeps the bottom of itself cut off for good.
            // Measured on the sticker pack preview: ContentElement 320x500 carrying
            // InsetClip(0,0,0,316), which left the title, a row and a half of stickers and no
            // Remove button at all, on a popup whose contents were laid out correctly underneath.
            // Batch_Completed is called from here for the same reason ShowHideTopTabs calls its
            // own completion by hand: CompositionScopedBatch.Completed never fires on Uno, so
            // CommandSpace would stay invisible and the redirect visual would stay attached.
            batch.EndWithCompleted(inset.Duration, () =>
            {
                content.Clip = null;
        ...
        protected override void OnApplyTemplate()
        {
            ...
            {
                // DismissButton lleva x:Load="False" en la plantilla (Themes/Generic.xaml), o sea
                // que hasta que alguien lo realiza NO es un boton: es un ElementStub. Y
                // GetTemplateChild se salta los ElementStub, mientras que FindName es lo que los
                // materializa. En Skia esto devolvia null y la linea siguiente lo desreferenciaba:
                // NullReferenceException dentro de OnApplyTemplate, o sea dentro de EnsureTemplate,
                // o sea dentro de ContentDialog.ShowAsync -- ver el comentario de ShowQueuedAsync
                // para por que esa excepcion no se veia por ningun lado.
                DismissButton = GetTemplateChild(nameof(DismissButton)) as Button
                    ?? FindName(nameof(DismissButton)) as Button;

                // Y aun asi se comprueba: una plantilla puede no traer la parte, y quedarse sin
                // boton de cerrar es un degradado; llevarse por delante el popup entero no lo es.
                ...
                {
                    Logger.Error($"{GetType().Name}: DismissButton no se pudo materializar");
                }
            }
        ...
        {
            var contentTemplateRoot = PrimaryButton.ContentRoot();
