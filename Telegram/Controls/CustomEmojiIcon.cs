...
namespace Telegram.Controls
{
    public partial class CustomEmojiIcon : AnimatedImage
    {
        ...
        {
#if LINUX
            // Uno's TextBlock takes no InlineUIContainer (and RichTextBlock is a stub on Skia):
            // the emoji's own text stands in for the player.
            inliness.Clear();

            if (message != null)
            {
                inliness.Add(new Run { Text = message.ReplaceSpoilers().Text });
            }
#else
        ...
        public static void Add(RichTextBlock parent, InlineCollection inliness, IClientService clientService, ChatFolderName name, double size = 20)
        {
            ...
            if (name?.Text != null)
            {
                inliness.Add(new Run { Text = name.Text.ReplaceSpoilers().Text });
            }
#else
            var direct = XamlDirect.GetDefault();
            var inlines = direct.GetXamlDirectObject(inliness);
