...
namespace Telegram.Controls
{
    public partial class TopNavView : ListViewEx
    {
        ...
        {
#if LINUX
            // Uno never raises ChoosingItemContainer (PORTING.md 6), which is the only place where
            // a list that wants a container other than the default one gets to say so - ForumView
            // wants ChatListListViewItem for its vertical list and TopNavViewItem for the two nav
            // modes. This is the hook that replaces it: same instant (the container is built here,
            // before PrepareContainerForItemOverride), and null keeps the Windows behaviour.
            SelectorItem container = ContainerFactory?.Invoke() ?? new TopNavViewItem();
#else
...
#if LINUX
        /// <summary>
        /// Builds the item container, in place of the <c>ChoosingItemContainer</c> handler that
        /// Uno never raises. Set it before the list is measured: <c>ContextRequested</c> is wired
        /// to whatever <see cref="ItemContextRequested"/> holds when the container is created.
        /// </summary>
        public Func<SelectorItem> ContainerFactory { get; set; }
#endif

        public event TypedEventHandler<UIElement, ContextRequestedEventArgs> ItemContextRequested;

        /// <summary>
        /// The handler <see cref="GetContainerForItemOverride"/> attaches to every container.
        /// A derived class that replaces the container factory WITHOUT calling base - which is
        /// what <c>ChatListListView</c> does - has to attach the same thing, and C# will not let
        /// it name the event itself outside this type (CS0070). So it attaches this.
        /// </summary>
        protected void RaiseItemContextRequested(UIElement sender, ContextRequestedEventArgs args)
        {
...
#if LINUX
                    // CompositionScopedBatch.Completed never fires on Uno (PORTING.md 6), and this
                    // is the handler that puts the two indicators back: opacity, offset, scale and
                    // the _prev/_next pair the next selection reads. Without it the outgoing
                    // indicator stays lit at its animated offset and every later selection has to
                    // force-complete the previous one. The animations above all run for 300 ms.
                    scopedBatch.EndWithCompleted(TimeSpan.FromMilliseconds(300), () => OnAnimationCompleted(null, null));
#else
                    scopedBatch.End();
#endif
    ...
    public partial class TopNavViewItemManager : VisualStateManager
    {
        ...
        {
            if (control is TopNavViewItem selector && selector.ContentRoot() is Control element)
            {
