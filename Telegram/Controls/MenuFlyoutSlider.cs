...
namespace Telegram.Controls
{
    ...
    public partial class MenuFlyoutSlider : MenuFlyoutItem
    {
        ...
        protected override void OnApplyTemplate()
        {
            ...
            // The only <Style TargetType="local:MenuFlyoutSlider"> is win:-only
            // (Themes/MenuFlyout_themeresources.xaml), so on Skia this control has no template at
            // all and Presenter is null. Without the two null checks OnApplyTemplate throws from
            // inside the menu's layout, which costs the whole flyout, not just its slider.
            if (Presenter != null)
            {
        ...
        {
            if (FocusState != FocusState.Unfocused && Presenter != null)
            {
                Presenter.Focus(FocusState);
            }
        }
