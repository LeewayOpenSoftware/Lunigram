    ...
    public partial class FormattedTextBlock : ISelectableControl
    {
...
#if LINUX
        // PARIDAD A8. No TextPointers on Uno Skia: TextBlock (what this control renders into
        // here, since Uno's RichTextBlock is [NotImplemented] for __SKIA__) has no
        // GetPositionFromPoint and no TextPointer, so there is no way to turn a pointer position
        // into a character index -- see the long note in MessageSelector.xaml.cs. Not a Phase 1
        // shortcut: it needs a text-measure pass of its own.
        ...
        public int ContentLength
        {
            get
            {
                ...
                {
#if LINUX
                    _contentLength = GetRenderedLength();
#else
                    var offsets = Offsets;
            ...
            get
            {
                if (_offsets == null)
                {
                    ...
                    if (TextBlock != null)
                    {
                        var index = 0;
#if LINUX
                        // Uno's RichTextBlock is a TextBlock with one Span per paragraph (see the
                        // field's own declaration): there is no Blocks collection and no ContentEnd,
                        // so the inlines are walked directly and the whole thing counts as block 0.
                        CollectOffsets(TextBlock.Inlines, entries, ref index, 0, false);
