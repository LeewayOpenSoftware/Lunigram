...
namespace Telegram.Controls
{
    public sealed partial class ProfileHeader : UserControl
    {
        ...
        private void InitializeAccessibleNames()
        {
            ...
            AutomationProperties.SetName(LinkedChannelButton, Strings.ProfileChannel);
...
#if LINUX
            // Writing a CompositionPropertySet notifies nobody in Uno, so the three scalars above
            // would not reach the expressions that read them until the next scroll. See
            // Telegram.Linux/Xaml/ProfileHeader.Linux.cs.
            InvalidateScrolling();
#endif
        }
...
#pragma warning restore CS0162
...
#if LINUX
            // Assigned rather than animated: neither animation sets Duration, and an unset Duration
            // is TimeSpan.Zero in Uno, which makes KeyFrameEvaluator answer the final value on its
            // first evaluation - the fade was already a jump. Assigning also sidesteps the
            // CompositionTarget trap that empties a control animated before it is attached. Same
            // reasoning as ProfilePage.ShowHideSubtitle; PORTING.md §6.
            subtitleTab.Opacity = show ? 1 : 0;
            subtitlePro.Opacity = show ? 0 : 1;
#else
...
#if LINUX
            // TWO traps meet on these two lines, and the first one throws.
            //
            // The same ScalarKeyFrameAnimation instance is started on two visuals. Uno's
            // Compositor.RegisterAnimation is _runningAnimations.Add(animation, target) on a
            // Dictionary KEYED BY THE ANIMATION, so the second StartAnimation is
            // "An item with the same key has already been added" - and unlike an ExpressionAnimation
            // (IsTrackedByCompositor false, so it returns before the dictionary) a KeyFrameAnimation
            // is tracked and really does get there. This is called from ViewChanged, so it would
            // throw out of the scroll handler on every profile the user scrolls.
            //
            // The second is the usual one: no Duration, hence TimeSpan.Zero, hence the final value
            // on the first evaluation. So the fade is a jump either way and assigning costs nothing
            // - and assigning is also immune to the CompositionTarget trap. PORTING.md §6.
            headerBackground.Opacity = show ? 0 : 1;
            headerGlow.Opacity = show ? 0 : 1;
#else
        ...
        public void InitializeScrolling(CompositionPropertySet properties)
        {
...
#if LINUX
            // Three substitutions, all forced by Uno and all measured; PORTING.md §6 has the detail.
            //
            // 1. ScrollY. There is no GetScrollViewerManipulationPropertySet, and the stand-in the
            //    port returns is a CompositionPropertySet, which never notifies when it is written:
            //    an expression that reads one is evaluated exactly once and then frozen. The scroll
            //    position is carried by a Visual instead (Offset.Y, same value and same sign as
            //    Translation.Y), because a Visual's setters do notify, and that notification is the
            //    only thing that makes an expression re-evaluate. See
            //    Telegram.Linux/Xaml/ElementCompositionPreview.cs.
            // 2. Set. The expression lexer only starts an identifier on a letter, so a leading
            //    underscore is "Unexpected character '_'" the moment the animation starts.
            // 3. Self. "this" is not in the lexer's vocabulary either: it lexes as a plain
            //    identifier and then fails to resolve, "Unrecognized identifier 'this'". Each of
            //    these expressions is started on exactly one visual, so the replacement is a normal
            //    reference parameter pointing at that visual.
            const string ScrollY = "scrollViewer.Offset.Y";
            const string Set = "props";
            const string Self = "self";
#else
            const string ScrollY = "scrollViewer.Translation.Y";
            const string Set = "_";
            const string Self = "this.Target";
#endif

            var translationExp = $"({ScrollY} - {Set}.RemovedHeight)";

            // What "scrollViewer" resolves to. On Windows it is the manipulation property set; on
            // Linux the paired carrier Visual, which is the half that notifies (see the block above).
            // Typed as CompositionObject because that is what SetReferenceParameter takes.
            CompositionObject scrolling = properties;
            var propertySet = Properties;

#if LINUX
            var carrier = ElementCompositionPreview.GetScrollingCarrier(properties);

            scrolling = carrier;
            _scrollingCarrier = carrier;
#endif

            // "this.Target" is replaced below by a reference parameter pointing at the visual the
            // animation is started on. Every expression here is started on exactly one visual, so
            // there is no ambiguity about what "this" meant.

            //var rootExp = "clamp(-scrollViewer.Translation.Y - (this.Target.Size.Y - 48 + 0), 0, target.Size.Y - this.Target.Size.Y)";
            var rootExp = $"{translationExp} < 0 ? clamp(-{translationExp} - ({Self}.Size.Y - 48), 0, 2147483647) : -{translationExp}";
            var rootTranslation = root.Compositor.CreateExpressionAnimation(rootExp);
            rootTranslation.SetReferenceParameter("scrollViewer", scrolling);
            rootTranslation.SetReferenceParameter("target", target);
            rootTranslation.SetReferenceParameter(Set, propertySet);
#if LINUX
            rootTranslation.SetReferenceParameter(Self, root);
#endif
            ...
            photoScale.SetReferenceParameter("scrollViewer", scrolling);
            photoScale.SetReferenceParameter("root", root);
            photoScale.SetReferenceParameter(Set, propertySet);

            var photoTranslation = root.Compositor.CreateExpressionAnimation($"clamp(-{translationExp} * 0.2, 0, 140)");
            photoTranslation.SetReferenceParameter("scrollViewer", scrolling);
            photoTranslation.SetReferenceParameter("root", root);
            photoTranslation.SetReferenceParameter(Set, propertySet);

            //var rootExp = "clamp(-scrollViewer.Translation.Y - (this.Target.Size.Y - 48 + 0), 0, target.Size.Y - this.Target.Size.Y)";
            var controlsExp = $"-{translationExp} - (target.Size.Y - 40)";
            var controlsClip = root.Compositor.CreateExpressionAnimation(controlsExp);
            controlsClip.SetReferenceParameter("scrollViewer", scrolling);
            controlsClip.SetReferenceParameter("target", root);
            controlsClip.SetReferenceParameter(Set, propertySet);

            //var buttonsExp = "clamp(-scrollViewer.Translation.Y - (target.Size.Y - this.Target.Size.Y - 56), 0, 72)";
            var buttonsExp = $"clamp(-{translationExp} - (target.Size.Y - {Self}.Size.Y - 56 - audio.Size.Y), 0, 72)";
            var buttonsTranslation = root.Compositor.CreateExpressionAnimation(buttonsExp);
            buttonsTranslation.SetReferenceParameter("scrollViewer", scrolling);
            buttonsTranslation.SetReferenceParameter("target", root);
            buttonsTranslation.SetReferenceParameter("audio", audio);
            buttonsTranslation.SetReferenceParameter(Set, propertySet);
#if LINUX
            buttonsTranslation.SetReferenceParameter(Self, buttons);
#endif

            var buttonsOpacity = root.Compositor.CreateExpressionAnimation($"clamp(1 - {buttonsExp} / {Self}.Size.Y, 0, 1)");
            buttonsOpacity.SetReferenceParameter("scrollViewer", scrolling);
            buttonsOpacity.SetReferenceParameter("target", root);
            buttonsOpacity.SetReferenceParameter("audio", audio);
            buttonsOpacity.SetReferenceParameter(Set, propertySet);
#if LINUX
            buttonsOpacity.SetReferenceParameter(Self, buttons);
#endif

            var audioExp = $"clamp(-{translationExp} - (target.Size.Y - buttons.Size.Y - 72 - {Self}.Size.Y), 0, 72)";
            var audioOpacity = root.Compositor.CreateExpressionAnimation($"clamp(1 - {audioExp} / {Self}.Size.Y, 0, 1)");
            audioOpacity.SetReferenceParameter("scrollViewer", scrolling);
            audioOpacity.SetReferenceParameter("target", root);
            audioOpacity.SetReferenceParameter("buttons", buttons);
            audioOpacity.SetReferenceParameter(Set, propertySet);
#if LINUX
            audioOpacity.SetReferenceParameter(Self, audio);
#endif
            ...
            titleTranslation.SetReferenceParameter("scrollViewer", scrolling);
            titleTranslation.SetReferenceParameter("buttons", buttons);
            titleTranslation.SetReferenceParameter("audio", audio);
            titleTranslation.SetReferenceParameter(Set, propertySet);
            ...
            titleScale.SetReferenceParameter("scrollViewer", scrolling);
            titleScale.SetReferenceParameter(Set, propertySet);
            ...
            subtitleScale.SetReferenceParameter("scrollViewer", scrolling);
            subtitleScale.SetReferenceParameter(Set, propertySet);

            if (ViewModel.IsSavedMessages)
            {
                ClipperBackground.Margin = new Thickness(0, -48, 0, -8);

                var clipperTranslation = root.Compositor.CreateExpressionAnimation($"-{ScrollY} + 32");
                clipperTranslation.SetReferenceParameter("scrollViewer", scrolling);
...
#if LINUX
                // Uno's expression lexer has no boolean AND. Its whole alphabet is
                // `+ - * / . , ( ) ? :` plus `< <= > >=` plus identifiers and numbers (measured on
                // the deployed Uno.UI.Composition: ExpressionAnimationLexer._knownTokens and the
                // two explicit '<' / '>' branches of EatToken -- there is no `&&`, no `||`, no `!`,
                // no `==`, no `!=` and no `%`). An `&&` throws ArgumentException "Unexpected
                // character '&'" out of StartAnimation, which here means out of
                // ProfileHeader.InitializeScrolling, which means out of ProfilePage.OnNavigatedTo:
                // the navigation dies and the profile is never shown at all.
                //
                // `a && b ? X : Else` is rewritten as `a ? (b ? X : Else) : Else`, which is the
                // same function of the same inputs -- these are pure arithmetic on visual
                // properties, so evaluating the common tail twice in the source text costs
                // nothing and changes nothing. The parser does accept a parenthesised ternary:
                // ParsePrimaryExpression on OpenParenToken recurses into a full ParseExpression.
                var clipperExpElse = $"-{translationExp} < target.Size.Y - 48 ? 0 : -{translationExp} < target.Size.Y - 24 ? {clipperExpClamp} : {clipperExpBranch1}";
                var clipperTranslation = root.Compositor.CreateExpressionAnimation($"{translationExp} < 0 ? (-{translationExp} > root.Size.Y - 48 ? (-{translationExp} < target.Size.Y - 48 ? {clipperExpBranch2} : ({clipperExpElse})) : ({clipperExpElse})) : -{translationExp}");
#else
...
#endif
                clipperTranslation.SetReferenceParameter("scrollViewer", scrolling);
                clipperTranslation.SetReferenceParameter(Set, propertySet);
            ...
            // titleTranslation deliberately runs on two visuals. That is the one shape of the
            // "an animation is not shared in this port" rule that does NOT apply: measured on the
            // deployed Uno.UI.Composition, Compositor.RegisterAnimation returns immediately unless
            // animation.IsTrackedByCompositor, and that is true for KeyFrameAnimation and false for
            // ExpressionAnimation, so no entry ever reaches the dictionary that throws on a
            // duplicate key. Both visuals subscribe to AnimationFrame and both re-evaluate.
        ...
        {
#if LINUX
            // The overload that takes a translate service walks a RichTextBlock's selection and
            // its Hyperlink run, and both halves are out here: RichTextBlock is [NotImplemented]
            // for __SKIA__ so these labels are TextBlocks (see the win:/not_win: pair in the XAML),
            // a TextBlock has no selection, and MessageHelper keeps only the three-argument
            // overload on Linux. Nothing to show, so no menu rather than an empty one.
#else
        ...
        private void Description_ContextRequested(UIElement sender, ContextRequestedEventArgs args)
        {
            ...
            {
#if LINUX
                // The text is already in hand, so the three-argument overload gives the real
                // Open / Copy menu; only the "translate" entry is lost with the service.
                MessageHelper.Hyperlink_ContextRequested(sender, description, args);
#else
...
#if LINUX
                    // Views/Popups/PlaybackPopup (the playlist sheet) is not in the subset -- the
                    // same reason PlaybackHeader.View_Click is guarded. The audio still plays and
                    // is still governed from the "now playing" band over the chat list.
#else
