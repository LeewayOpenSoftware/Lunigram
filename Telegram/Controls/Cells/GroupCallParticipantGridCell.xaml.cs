...
namespace Telegram.Controls.Cells
{
    public sealed partial class GroupCallParticipantGridCell : HyperlinkButton
    {
        ...
        // u-groupcall-compile-clean: these five are `internal`, not `public`, ON PURPOSE.
        // Uno's BindableTypeProvidersSourceGenerator walks PUBLIC properties and emits
        // `typeof(global::<SimpleName>)` for TDLib-generated types it cannot see (source
        // generators do not observe each other's output), producing CS0400 in
        // BindableMetadata.g.cs. Nothing binds these from XAML; all consumers are C# in
        // this same assembly, so `internal` keeps every call site working and takes them
        // out of the generated metadata. Same root cause as the ReactionsMenuFlyout
        // ItemClick fix in GroupCallWindow (700ed92).
        internal SpriteVisual Visual => _visual;

        internal VoipVideoChannelQuality Quality => ActualHeight switch
        {
        ...
        internal GroupCallParticipant Participant => _participant;
        internal MessageSender ParticipantId => _participant.ParticipantId;

        internal GroupCallParticipantVideoInfo VideoInfo => _videoInfo;
...
#if LINUX
        private Telegram.Controls.Calls.VoipVideoRenderer _renderer;

        public void SetVideoRenderer(Telegram.Controls.Calls.VoipVideoRenderer renderer, bool mirrored = false)
        {
            if (_renderer == renderer && CanvasRoot.Child == renderer?.Element)
            {
                CanvasRoot.RenderTransformOrigin = new Windows.Foundation.Point(0.5, 0.5);
                CanvasRoot.RenderTransform = mirrored
                    ? new Microsoft.UI.Xaml.Media.ScaleTransform { ScaleX = -1 }
                    : null;
                return;
            }

            if (CanvasRoot.Child is FrameworkElement)
            {
            ...
            {
                if (renderer.Element.Parent is Border parentBorder)
                {
                    parentBorder.Child = null;
                }
                else if (renderer.Element.Parent is Panel parentPanel)
                {
                    parentPanel.Children.Remove(renderer.Element);
                }

                renderer.Element.HorizontalAlignment = HorizontalAlignment.Stretch;
                renderer.Element.VerticalAlignment = VerticalAlignment.Stretch;
                renderer.Element.Visibility = Visibility.Visible;
                ...
                CanvasRoot.Child = renderer.Element;
            }
            else
            {
                CanvasRoot.RenderTransform = null;
            }
        }
#endif
        ...
        private void ShowHidePaused(bool show)
        {
            ...
            {
#if LINUX
                OverlayBlur.Background = new SolidColorBrush(Color.FromArgb(0xCC, 0x00, 0x00, 0x00));
                OverlayRoot.Visibility = Visibility.Visible;
