...
#if !LINUX
// Telegram/ViewModels/Stories is not compiled in the Linux subset.
...
namespace Telegram.Controls.Cells
{
    public sealed partial class ProfileCell : ContentControl
    {
...
#if !LINUX
        // ActiveStoriesViewModel is Telegram.ViewModels.Stories, and the stories subsystem is out
        // of the Linux subset: the using at the top of this file does not resolve there.
        ...
        // The three phase bodies below were lifted out of UpdateSearchResult without a change of
        // behaviour, so that the Linux half of this class can run all three in one pass: Uno
        // raises ContainerContentChanging once per container with Phase always 0, and
        // RegisterUpdateCallback is a no-op, so phases 1 and 2 never arrive on their own and a
        // search result would be drawn with its title and nothing else - no "last seen", no
        // avatar. See Telegram.Linux/Xaml/ProfileCell.Linux.cs.

        private void UpdateSearchResultPhase0(IClientService clientService, SearchResult result)
        {
            RecycleSearchResult();
        ...
        private void UpdateSearchResultPhase1(IClientService clientService, SearchResult result)
        {
        ...
        private void UpdateSearchResultPhase2(IClientService clientService, SearchResult result)
        {
            if (result.Chat != null)
            {
        ...
        public void UpdateStatisticsByChat(IClientService clientService, ContainerContentChangingEventArgs args, TypedEventHandler<ListViewBase, ContainerContentChangingEventArgs> callback)
        {
            ...
            // ContentRoot() and not ContentTemplateRoot: the latter is null for every templated
            // container in Uno (PORTING.md 6). `content` is unused below, so the miss would be
            // invisible either way, but leaving a live ContentTemplateRoot in the subset is how
            // the next reader learns the wrong pattern.
            var content = args.ItemContainer.ContentRoot() as ProfileCell;
