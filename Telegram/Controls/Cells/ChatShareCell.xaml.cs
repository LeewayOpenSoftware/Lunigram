    ...
    public sealed partial class ChatShareCell : ControlEx, IMultipleElement
    {
        ...
        public void UpdateState(bool selected, bool animate, bool multiple)
        {
            ...
            if (animate)
            {
                ...
                // One instance per target: this fades BOTH _visual and _selectionOutline, and Uno
                // keys its running-animation dictionary by the animation INSTANCE, so the second
                // start would throw ArgumentException and take the dispatcher with it. Every
                // selection toggle in the share/forward popup runs this. See ButtonEx.ShowSkeleton
                // for the measurement behind it.
                ScalarKeyFrameAnimation CreateOpacity()
                {
                    var animation = compositor.CreateScalarKeyFrameAnimation();
                    animation.InsertKeyFrame(selected ? 0 : 1, 0);
                    animation.InsertKeyFrame(selected ? 1 : 0, 1);
                ...
                _visual.StartAnimation("Opacity", CreateOpacity());
                ...
                _selectionOutline.StartAnimation("Opacity", CreateOpacity());
            }
            else
            {
