...
#if LINUX
// No XamlDirect on Uno: the builder runs over the managed Run/Span/Hyperlink objects
// through TextElementDirect (Telegram.Linux/Xaml), which keeps the same call shape.
using IXamlDirectObject = System.Object;
using XamlDirect = Telegram.Controls.TextElementDirect;
using XamlPropertyIndex = Telegram.Controls.TextElementProperty;
using XamlTypeIndex = Telegram.Controls.TextElementType;
#else
...
namespace Telegram.Controls
{
    ...
    public partial class FormattedTextBlock : FormattedTextBlockBase
    {
...
#if LINUX
        // Never built here (no Win2D): stays null, which turns the spoiler hit-testing off.
        private object _spoilerGeometry;
#else
...
#if LINUX
        // Uno's RichTextBlock is a stub on Skia: a TextBlock, one Span per paragraph.
        ...
        {
#if LINUX
            // A TextBlock has no Blocks: only XAML-declared Paragraphs are ever listed here.
            get => _blocks ??= new List<Block>();
#else
        ...
        protected override void OnApplyTemplate()
        {
...
#if LINUX
            TextBlock = GetOrCreateTextBlock();
#else
            ...
            if (TextBlock != null && _blocks != null)
            {
                for (int i = 0; i < _blocks.Count; i++)
                {
...
#if LINUX
                    // AdoptParagraph *moves* the inlines into the TextBlock and leaves the
                    // paragraph empty, so the trailing Span has to be read before the move -
                    // reading it after left _spanForInlines null, and then SetText appended a
                    // whole new paragraph instead of filling that Span: the preview came out as
                    // "sender:" + a line break + the text (invisible with MaxLines="1"), and the
                    // next time round the clear-all path detached the sender Run for good.
                    var trailing = block.Inlines.Count > 0 ? block.Inlines[^1] : null;

                    AdoptParagraph(block);
#else
                    ...
                    if (i == _blocks.Count - 1 && trailing is Span spanForInlines)
                    {
...
#if LINUX
            if (TextBlock?.Inlines.Count > 0 && TextBlock.Inlines[0] is Span existing)
#else
        ...
        private void ApplyHighlighters()
        {
            ...
            if (_text != null && _query?.Length > 0 && _last >= _first)
            {
                ...
                if (find != -1)
                {
                    ...
                    {
#if LINUX
                        start += GetContentOffset(_spanForInlines);
#else
...
#if !LINUX
            // Uno's TextBlock paints the first highlighter's first range only, in white, so
            // the spoiler/cached/marked ones would do more harm than good on that side: the
            // spoiler text is hidden by its font (and revealed by its Hyperlink) instead.
            if (_marked != null)
            {
...
#if !LINUX
        // RichText flattening (PageBlockHelper) needs TDLib types the public schema lacks.
        ...
        private IXamlDirectObject GetOrCreateRun(XamlDirect direct, IXamlDirectObject inlines, string text, int offset, int length, FlowDirection direction, TextStyle style, FontFamily fontFamily, double fontSize, string prefix = null)
        {
            ...
            // 12.10 replaced the `transparent` flag with `prefix` (the zero-width character an
            // inline object leaves behind); spoilers go through the hyperlink path now, so nothing
            // asks for a transparent run any more.
#if LINUX
            run = prefix == null
                ? XamlDirect.AddRunToCollection(direct, inlines, text, offset, length, direction, style, fontFamily, fontSize, false)
                : XamlDirect.AddRunToCollection(direct, inlines, prefix + text.Substring(offset, length), direction, style, fontFamily, fontSize, false);
#else
            run = prefix == null
...
#if LINUX
            run = XamlDirect.AddRunToCollection(direct, inlines, text, direction, style, fontFamily, fontSize, false);
#else
                ...
                for (int j = 0; j < runs.Count; j++)
                {
                    ...
                    else
                    {
                        ...
                        if (paragraph != null)
                        {
                            ...
                            {
#if LINUX
                                // No particles overlay on this side: the spoiler font hides the
                                // glyphs and a Hyperlink lets a click reveal them (Entity_Click).
                                // 12.10 pools ProjectedHyperlink rather than the Hyperlink itself:
                                // .Element is the managed link, .Native the XamlDirect handle.
                                var hyperlink = GetOrCreateHyperlink(direct);
                                hyperlink.Element.UnderlineStyle = UnderlineStyle.None;
                                MessageHelper.SetHyperlinkInfo(hyperlink.Element, new TextEntityClickEventArgs(new TextEntityTypeSpoiler()));

                                IXamlDirectObject span = hyperlink.Native;
#else
...
#if LINUX
                                hyperlink.Element.Foreground = CodeForeground;
#endif
                        ...
                        else if (_ignoreSpoilers is false && entity.HasFlag(Native.TextStyle.Spoiler))
                        {
                            ...
                            {
#if LINUX
                                textOffset = GetContentOffset(_spanForInlines);
#else
                        ...
                        // !!!
#if LINUX
                        // Uno's TextBlock takes no InlineUIContainer: the emoji's own text stands
                        // in for the player, through the plain run at the end of the chain.
                        if (entity.Type is TextEntityTypeCustomEmoji customEmoji && InlineUIContainerSupported)
#else
...
#if !LINUX
        // InlineButton is not in the public TDLib schema yet.
        ...
        {
#if !LINUX
            // The particles overlay needs Win2D geometry and DirectWrite range metrics; on
            // Linux the spoiler is the font plus a click-to-reveal Hyperlink (see SetText).
        ...
        {
#if LINUX
            // The skeleton clip comes from LineMetrics/GetRoundedPolygon, which the Linux
            // helper doesn't provide yet: no skeleton, the text just appears.
            return;
#endif
...
#if LINUX
                // Paragraphs are Spans of a plain TextBlock here and Uno has no usable TextPointer
                // geometry, so the skeleton gets no line rectangles (an empty clip, nothing drawn).
