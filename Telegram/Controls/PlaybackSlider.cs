...
namespace Telegram.Controls
{
    ...
    public partial class PlaybackSlider : Control
    {
        ...
        public void UpdateValue(TimeSpan position, TimeSpan duration, bool playing)
        {
...
#if LINUX
            // Uno's expression lexer has no '_' in its alphabet: the property set named `_` makes
            // ExpressionAnimationLexer.EatToken throw ArgumentException ("Unexpected character
            // '_'") the moment the animation starts. That throw comes out of OnApplyTemplate,
            // which runs inside MeasureOverride, so it is one of the fatal ones (PORTING.md §6):
            // every voice note in the history took the whole panel down with it. One letter.
            const string set = "props";
#else
            const string set = "_";
#endif

            var progressAnimation = compositor.CreateExpressionAnimation($"visual.Size.X - ({set}.Progress * visual.Size.X)");
            progressAnimation.SetReferenceParameter(set, _props);
            ...
            {
                var thumbAnimation = compositor.CreateExpressionAnimation($"{set}.Progress * visual.Size.X");
                thumbAnimation.SetReferenceParameter(set, _props);
...
#if LINUX
                // And `this` is the other identifier Uno's parser does not know (PORTING.md §6):
                // the tooltip visual goes in as an ordinary reference parameter.
                var toolTipAnimation = compositor.CreateExpressionAnimation($"Vector3({set}.Progress * visual.Size.X - target.Size.X / 2, -target.Size.Y - 8, 0)");
                toolTipAnimation.SetReferenceParameter("target", toolTip);
#else
                var toolTipAnimation = compositor.CreateExpressionAnimation($"Vector3({set}.Progress * visual.Size.X - this.Target.Size.X / 2, -this.Target.Size.Y - 8, 0)");
#endif
                toolTipAnimation.SetReferenceParameter(set, _props);
                toolTipAnimation.SetReferenceParameter("visual", visual);

                toolTip.StartAnimation("Offset", toolTipAnimation);

                ThumbToolTip.Shadow = new Microsoft.UI.Xaml.Media.ThemeShadow();
