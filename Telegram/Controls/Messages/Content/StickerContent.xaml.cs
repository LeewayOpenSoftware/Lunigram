...
namespace Telegram.Controls.Messages.Content
{
    public sealed partial class StickerContent : HyperlinkButton, IContent, IPlayerView
    {
        ...
        private async void Button_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (_message.Content is MessageAnimatedEmoji animatedEmoji)
            {
                if (Player.IsPlaying == false)
                {
                    ...
                    // CERRADO 2026-08-27 (PARIDAD M25). La guarda venia de cuando SoundEffects era
                    // Windows.Media.Audio.AudioGraph y no existia sustituto: hoy
                    // Telegram.Linux/Native/Media/SoundEffects.cs tiene el mismo namespace, la
                    // misma clase y las mismas dos sobrecargas de Play, incluida Play(Td.Api.File),
                    // sobre libmpv. La animacion del emoji si corria; lo que faltaba era el sonido.
                    var sound = animatedEmoji.AnimatedEmoji.Sound;
            ...
            else
            {
                ...
                {
                    // PARIDAD M25: en Linux OpenSticker no abre nada. NO se puede cablear -- el
                    // destino es StickersPopup, y ni el popup ni StickersViewModel,
                    // AnimatedListHandler, ZoomableListHandler o FluidGridView entran en el
                    // subconjunto. Y tampoco hay nada que colapsar: el sticker es contenido, no un
                    // boton, asi que la unica salida honesta es que el toque quede sin efecto y
                    // quede escrito aqui. Lo que si se ha colapsado es el UNICO boton que este
                    // fichero dibuja apuntando aqui: el «Ver» del aviso de sticker premium
                    // (ShowToast, mas abajo).
                    _message.Delegate.OpenSticker(sticker);
                }
                else
        ...
        private async void ShowToast(MessageViewModel message, Sticker sticker)
        {
            ...
            {
#if LINUX
                // COLAPSADO 2026-08-27 (PARIDAD M25). El aviso se queda, con el nombre del pack y
                // su sticker; lo que se quita es el boton «Ver», porque lo unico que hace es
                // llamar a OpenSticker y OpenSticker no abre nada en esta cabeza (StickersPopup
                // fuera del subconjunto). Un aviso que informa vale; un boton que se dibuja y no
                // hace nada, no.
                ToastPopup.Show(XamlRoot, string.Format("**{0}**\n{1}", stickerSet.Title, Strings.PremiumStickerTooltip), new DelayedFileSource(message.ClientService, sticker));
#else
