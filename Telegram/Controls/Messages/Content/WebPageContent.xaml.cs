...
namespace Telegram.Controls.Messages.Content
{
    public sealed partial class WebPageContent : HyperlinkButton, IContentWithPlayback
    {
        ...
        private void UpdateContent(MessageViewModel message, LinkPreview linkPreview, bool small)
        {
            ...
            // 2026-08-27: this used to be a Linux-only ladder of three branches - document,
            // sticker, and twelve types to PhotoContent but only when !small. `small` had no
            // branch at all, so a compact preview (the 44x44 thumbnail beside the title, which is
            // what a link with a thumbnail and no ShowLargeMedia gets) always ended at
            // Media.Child = null, and a link to a video, a GIF, an audio file or a wallpaper
            // arrived with no picture. It is upstream's ladder again now: the six media branches
            // (Album, Animation, Audio, Video, VideoNote, VoiceNote), the story album, the three
            // embedded players with their real-media halves, the background and the upgraded gift
            // all resolve to controls that are compiled into this head. See the csproj additions
            // that come with this change: ThumbnailContent, StickerSetContent and WallpaperContent
            // had to be linked in; the rest were already here.
            if (small)
            {
    ...
    /// <remarks>
    /// Was #if !LINUX, together with the LinkPreviewTypeUpgradedGift branch that builds it. Its
    /// three ingredients are all in this head: PatternBackground (Controls/PatternBackground.xaml),
    /// AnimatedImage, and DelayedFileSource.FromSticker. What the port loses here is only the
    /// tinting of the pattern - PARIDAD.md section 2.3 has that measured and argued
    /// (ReplacementColor captures an empty surface in Uno) - and a gift preview with an untinted
    /// pattern is still the gift, where no preview at all was nothing.
    /// </remarks>
