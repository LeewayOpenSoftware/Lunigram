...
namespace Telegram.Controls.Messages.Content
{
    public sealed partial class PollOptionContent : ToggleButtonEx
    {
        ...
        private Microsoft.UI.Xaml.Controls.ProgressBar Votes;
        ...
        protected override void OnApplyTemplate()
        {
            ...
            Votes = GetTemplateChild(nameof(Votes)) as Microsoft.UI.Xaml.Controls.ProgressBar;
        ...
        // u004 / PORTING.md §6: Uno's BindableTypeProviders generator can't qualify a type that
        // another generator produced (Telegram.Generators from td_api.tl) and emits
        // typeof(global::PollOption) -- CS0400. Only walks PUBLIC properties, so internal on
        // Linux dodges it; every consumer is in this same assembly.
#if LINUX
        internal PollOption Option { get; private set; }
#else
        ...
        private void Media_Click(object sender, RoutedEventArgs e)
        {
            ...
            else
            {
                ...
                if (media != null)
                {
                    // u004: IMessageDelegate.OpenMedia(GalleryMedia, ...) is #if !LINUX only --
                    // the Linux gallery pipeline doesn't have an entry point for a bare
                    // GalleryMedia yet (only the MessageViewModel overload). Pre-existing gap,
                    // outside u004's scope (13 empty message bubbles); tapping a quiz-poll
                    // option's own photo/animation/video answer is a no-op here rather than a
                    // silent crash or a wrong open.
#if !LINUX
