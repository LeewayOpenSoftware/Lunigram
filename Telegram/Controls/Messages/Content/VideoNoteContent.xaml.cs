...
#endif
            // Uno's Shape.GetAlphaMask() throws NotImplementedException, and this runs on a path
            // the user walks (highlighting a message: jumping to a reply or a search result).
            // MessageBubble.Highlight already treats null as "no mask", which just means the
            // highlight is the bubble rectangle instead of the circle.
            return null;
        }
...
#if !LINUX
                // SwapChainPanel is one of the types Uno declares and does not implement, and the
                // panel only exists to give the media player a surface to draw the playing note
                // on. With the inert playback service CurrentItem is always null, so this branch
                // is unreachable on Linux anyway; it comes back with the player in phase 5.
                if (_panel == null)
                {
