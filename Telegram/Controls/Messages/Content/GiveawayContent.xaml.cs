...
#if LINUX
        private TextBlock PrizesHeader;
        private TextBlock ParticipantsHeader;
        private TextBlock WinnersHeader;
#endif
...
#if LINUX
            // PORTING.md §6: {CustomResource} on an attached property mis-compiles under Uno
            // (CS1061). These three headers are win:/not_win:-duplicated in the XAML; fill the
            // Skia copies here instead of through the attached property.
            PrizesHeader = GetTemplateChild(nameof(PrizesHeader)) as TextBlock;
            ParticipantsHeader = GetTemplateChild(nameof(ParticipantsHeader)) as TextBlock;
            WinnersHeader = GetTemplateChild(nameof(WinnersHeader)) as TextBlock;

            TextBlockHelper.SetMarkdown(PrizesHeader, PrizesHeader.Inlines, Strings.BoostingGiveawayPrizes);
            TextBlockHelper.SetMarkdown(ParticipantsHeader, ParticipantsHeader.Inlines, Strings.BoostingGiveawayMsgParticipants);
            TextBlockHelper.SetMarkdown(WinnersHeader, WinnersHeader.Inlines, Strings.BoostingWinnersDate);
#endif
