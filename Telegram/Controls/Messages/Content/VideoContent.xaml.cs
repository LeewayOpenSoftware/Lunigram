        ...
        private void UpdateFile(MessageViewModel message, File file, Video video, AlternativeVideo lowQuality, bool hasSpoiler, bool isSecret)
        {
            ...
            else
            {
                ...
                else if (state == MessageContentState.Download)
                {
                    ...
                    {
#if !LINUX
                        // VideoPreloader drives an AsyncMediaPlayer just to fill the stream cache,
                        // and on Linux that player is the inert phase 1 stub: it would spin up a
                        // thread per video and never buffer anything. Preloading comes back with
                        // the real player in phase 5.
