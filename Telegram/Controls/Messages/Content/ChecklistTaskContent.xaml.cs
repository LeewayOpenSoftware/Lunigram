...
namespace Telegram.Controls.Messages.Content
{
    public sealed partial class ChecklistTaskContent : ToggleButtonEx
    {
        ...
        // typeof(global::ChecklistTask) -- CS0400. Only walks PUBLIC properties, so internal on
        // Linux dodges it; every consumer is in this same assembly.
#if LINUX
        internal ChecklistTask Task { get; private set; }
#else
        ...
        public void UpdateChecklistTask(MessageViewModel message, Checklist checklist, ChecklistTask task)
        {
            ...
            if (checklist.CanMarkTasksAsDone && recycled && !_completedBy.AreTheSame(task.CompletedBy))
            {
                ...
                photo.StartAnimation("Opacity", CreateOpacity());
                userText.StartAnimation("Opacity", CreateOpacity());
