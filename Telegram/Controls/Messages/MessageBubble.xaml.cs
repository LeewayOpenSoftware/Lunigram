...
namespace Telegram.Controls.Messages
{
    ...
    public sealed partial class MessageBubble : ControlEx, IReactionsDelegate
    {
        ...
        public void Recycle()
        {
            // A bubble can be recycled before its template has ever been applied, and then these
            // three are still null. On Uno that is not rare: the container enters the tree inside
            // VirtualizingPanelLayout's fill pass, and a fast scroll recycles some of them before
            // OnApplyTemplate has run -- measured twice per session in the app, each time as a
            // NullReferenceException that aborted the rest of the recycling (the footer, the
            // media content and UnregisterEvents), so the row went back to the pool half cleaned.
            // Elsewhere in this file the same fields are already read with a null check.
            Message?.Clear();
...
#if LINUX
            ProbeRightEdgeOnce(message);
#endif
        }

#if LINUX
        private static bool _rightEdgeProbed;

        /// <summary>
        /// Measures, once per run, where an outgoing media bubble's right edge actually lands.
        /// </summary>
        /// <remarks>
        /// Jim's step (a) for u-media-bubble-right-edge. It answers ONE question: is the bubble
        /// overhanging its list by exactly its right margin - which would mean the inset is being
        /// dropped - or by more, which means the row itself is being measured wider than the
        /// viewport and the inset is a red herring.
        ///
        /// Worth reading before trusting the prediction: Uno's own FrameworkElement.
        /// InnerArrangeCore (decompiled from Uno.UI 6.6.184) computes, for a Right-aligned child,
        /// x = (finalRect.Width - margin.Left - margin.Right - renderSize.Width) + finalRect.X +
        /// margin.Left, so the child's right edge is finalRect.Right - margin.Right for ANY
        /// renderSize - the margin cannot be dropped there. So the expected answer here is "more
        /// than the margin", and this measures it rather than assuming it.
        ///
        /// Once per run and only for outgoing media: this is on the layout path of every message.
        /// </remarks>
        private void ProbeRightEdgeOnce(MessageViewModel message)
        {
            if (_rightEdgeProbed || message?.Content is not (MessagePhoto or MessageVideo or MessageAnimation))
            {
                return;
            }

            if (!message.IsOutgoing && message.ForwardInfo?.Source is not { IsOutgoing: true })
            {
                return;
            }

            _rightEdgeProbed = true;
            ...
            void handler(object sender, object e)
            {
                ...
                try
                {
                    ...
                    while (parent != null && list == null)
                    {
                        parent = VisualTreeHelper.GetParent(parent);
                        list = parent as ListViewBase;
                    }

                    if (list == null)
                    {
                        Logger.Info($"bubble right edge: margin {Margin.Left},{Margin.Right} desired {DesiredSize.Width:F0} actual {ActualWidth:F0}; no list ancestor to measure against");
                        return;
                    }

                    var left = TransformToVisual(list).TransformPoint(default).X;
                    var right = left + ActualWidth;
                    var overhang = right - list.ActualWidth;

                    // The whole point of the probe is this comparison, so it is spelled out rather
                    // than left for a reader to subtract.
                    Logger.Info($"bubble right edge: margin.Right {Margin.Right:F0} | bubble left {left:F0} width {ActualWidth:F0} (desired {DesiredSize.Width:F0}) right {right:F0} | list width {list.ActualWidth:F0} | overhang {overhang:F0} -- {(Math.Abs(overhang - Margin.Right) < 1 ? "EQUALS the right margin: the inset is being dropped" : overhang > Margin.Right ? "MORE than the right margin: the row is wider than the viewport, different class" : "no overhang: the inset is being honoured here")}");
                }
                catch (Exception ex)
                {
                    Logger.Error(ex, "bubble right edge: probe failed");
                }
            }
...
#if LINUX
            // Esta variable desplaza una fila el Footer (la hora) y solo tiene sentido si delante
            // hay un MessageFactCheck o un MessageSummary. En Linux ese elemento NUNCA se inserta
            // -- el bloque que lo mete en Panel.Children[0] esta bajo #if !LINUX mas abajo en este
            // mismo metodo, y MessageBubblePanel sustituye su deteccion por `if (false)` en dos
            // sitios. Con la fila reservada para un elemento que no se dibuja, la hora de un mensaje
            // con verificacion de datos o con resumen se colocaba una fila por debajo de su sitio.
            var factCheck = 0;
#else
        ...
        public void UpdateMessageContentControl(MessageViewModel message)
        {
...
#if LINUX
            // Phase 1 contents: photo, album, sticker, document, link preview, video, GIF, video
            // note, audio, location -- and, since phase 5, voice notes. Audio and voice notes now
            // PLAY: the shared Services/PlaybackService.cs is live on Linux too, over libmpv
            // (Telegram.Linux/Native/Media/). Video and video notes still only draw their preview
            // (thumbnail, duration, size, download button); the video surface is phase 5 step 5.
            // Video and GIF do animate their preview through AnimatedImage, which is the same path
            // the WebM stickers already use.
            // u004: the 13 remaining empty-bubble content types (poll, contact, call log, to-do
            // list, giveaway, live location, venue, dice, invoice, game, payment-media preview,
            // sponsored, stake dice) -- none has a native dependency, ported straight off the
            // #else branch below. Same relative order as that branch so the two stay diffable.
            Media.Child = content switch
            {
                ...
                // Dice and stake-dice both play through controls:DiceView, which recolors and
                // composites the roll/slot-machine frames with Win2D CanvasControl +
                // Windows.Data.Json -- both unsupported by Uno Skia (PORTING.md §6). u-diceview:
                // MessageDice ported, backed by Telegram.Linux/Graphics/CanvasControl.cs (a
                // WriteableBitmap + Telegram.Native.PixelBuffer stand-in for CanvasControl/
                // CanvasBitmap, the same technique Controls/AnimatedImage.cs already uses for
                // sticker frames) and Telegram.Linux/Graphics/WindowsDataJson.cs (a
                // System.Text.Json.Nodes adapter for the JsonObject/JsonArray surface MergeReels
                // needs). MessageStakeDice stays collapsed: StakeDiceContent's Button_Click
                // constructs Views/Popups/StakeDicePopup.xaml.cs (881 lines, its own ViewModel/
                // dependencies), which is not in this subset and is out of this parcel's
                // boundary -- a separate tanda.
                MessageDice => new DiceContent(message),
                MessageStakeDice => new MessageUnsupportedContent(),
        ...
        protected override void OnLoaded()
        {
            ...
            // Same pre-template race OnUnloaded above is guarded against: ContentPanel/Message are
            // template children, null until OnApplyTemplate runs, which this can precede.
            ContentPanel?.SizeChanged += OnSizeChanged;
            Message?.TextEntityClick += Message_TextEntityClick;
        ...
        {
            // OnLoaded calls this before ContentPanel/Message are assigned when it runs ahead of
            // OnApplyTemplate (see the "TODO: a more elegant way is needed" call above) - both are
            // template children, null until then, same as every other member here already guarded.
            ContentPanel?.SizeChanged -= OnSizeChanged;
            Message?.TextEntityClick -= Message_TextEntityClick;
...
#if LINUX
            var direction = selector.Owner.CurrentScrollingMode == ItemsUpdatingScrollMode.KeepItemsInView ? -1 : 1;
#else
        ...
        public Rect Highlight(MessageBubbleHighlightOptions options)
        {
            ...
            // No RemoveAll here: _highlight is built fresh a few lines above, so the collection is
            // empty and there is nothing to remove - and asking anyway throws. Uno's RemoveAll
            // raises a Reset notification carrying a list of the items it removed, and a Reset is
            // only allowed to carry none, so the event's own constructor rejects it even when that
            // list is empty. It took out every jump to a message: a reply, a search result, a
            // mention.
            ...
            // Checklists and polls are still outside the Linux subset, so only the album arm of
            // this chain can be compiled there.
#if !LINUX
        ...
        public void UpdateMockup(IClientService clientService, Chat chat, MessageSender sender, string tag, ChatMemberRank rank)
        {
            ...
            {
#if LINUX
                var message = new Message(chat.Id, sender, null, 0, null, null, false, false, false, false, false, false, false, false, false, false, 0, 0, null, null, null, Array.Empty<UnreadReaction>(), null, null, null, null, null, 0, 0, 0, null, 0, 0, string.Empty, 0, string.Empty, 0, 0, null, string.Empty, null, null, 0);
#else
