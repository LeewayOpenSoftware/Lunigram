...
#if LINUX
            // ReactionAsTagButton is in the subset: everything it needs -- InputPopup, MessageTag,
            // Icons.Tag*, SetSavedMessagesTagLabel -- was already compiled, and it borrows
            // ReactionButton's template because its own is win:-only (see its constructor).
            // Without it the SAVED MESSAGES TAGS, which are the way saved messages get organized,
            // drew as ordinary reactions: counted instead of labelled, with the reaction menu
            // instead of rename/remove.
            //
            // ReactionAsPaidButton is NOT: its click spends stars through PaidReactionService and
            // its context menu is ReactPopup, and neither is in the subset. Paid reactions keep
            // drawing as plain reactions, which is a declared gap and not a way to spend stars by
            // accident: a plain reaction's click is addMessageReaction, and td_api says of its
            // reaction_type "Use addPendingPaidMessageReaction instead to add the paid reaction".
            button = isTag
