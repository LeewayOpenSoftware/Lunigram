...
namespace Telegram.Controls.Messages
{
    public sealed partial class MessageFooter : ControlEx
    {
...
#if LINUX
            // Visual.RelativeOffsetAdjustment is [NotImplemented] in Uno 6.6.184 - the string
            // "The member Vector3 Visual.RelativeOffsetAdjustment is not implemented" is in the
            // deployed Uno.UI.Composition.dll - and it neither throws nor warns nor stores the
            // value, so the ticks never travelled to the right edge of the label: they were drawn
            // from its origin, i.e. ON TOP OF the first characters of the time, with the 22px
            // reserved for them to the right left empty. Both AnchorPoint and the relative
            // adjustment are replaced by an absolute offset, kept up to date from the label's own
            // SizeChanged - its width changes with every time string and with "edited".
            container.Offset = new Vector3((float)Label.ActualWidth - width, 3, 0);

            if (!_ticksAnchored)
            {
                _ticksAnchored = true;
                Label.SizeChanged += OnTicksLabelSizeChanged;
            }
#else
...
#if LINUX
        private bool _ticksAnchored;

        private void OnTicksLabelSizeChanged(object sender, SizeChangedEventArgs e)
        {
            if (_container != null)
            {
                var offset = _container.Offset;
                offset.X = (float)e.NewSize.Width - _container.Size.X;
        ...
        /// <summary>
        /// Releases the tick geometries from any animation still registered on them, so the plain
        /// assignments that follow are the value that gets drawn.
        /// </summary>
        /// <remarks>
        /// On Uno a KeyFrameAnimation that has ALREADY FINISHED keeps driving its property -
        /// <c>Compositor.RenderRootVisual</c> re-evaluates it on every frame - so writing the final
        /// value on top of it without stopping it first has no effect (PORTING.md section 6). It
        /// matters here because <see cref="MessageFooter"/> is recycled with its container while
        /// <c>_line11</c>/<c>_line12</c>/<c>_line21</c> are instance fields that
        /// <c>InitializeTicks</c> only ever builds once: a footer that animated to Read and is then
        /// bound to a message that is merely Sent would otherwise keep the double tick.
        /// </remarks>
        private void StopTicks()
        {
#if LINUX
            _line11?.StopAnimation("TrimEnd");
            _line12?.StopAnimation("TrimEnd");
            _line21?.StopAnimation("TrimStart");
            _line21?.StopAnimation("TrimEnd");
            _line22?.StopAnimation("TrimEnd");
#endif
        }
