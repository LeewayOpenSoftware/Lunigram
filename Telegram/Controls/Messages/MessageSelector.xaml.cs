...
namespace Telegram.Controls.Messages
{
    ...
    public sealed partial class MessageSelector : ToggleButtonEx2
    {
...
#if LINUX
        // PARIDAD A8, revisado el 2026-08-27. El comentario anterior decia que
        // Common/TextSelectionManager.cs no estaba en el subconjunto: ESO YA NO ES CIERTO, el
        // fichero se compila (Telegram.Linux.csproj) y TranslatePopup lo usa. El motivo real es
        // otro y esta un nivel mas abajo, en el unico dato que el gestor necesita y que aqui no
        // existe: PUNTO DE PANTALLA -> INDICE DE CARACTER.
        //
        // FormattedTextBlock.GetPositionFromPoint devuelve 0 bajo #if LINUX y
        // IsSelectionEnabled contesta false (FormattedTextBlock.Selectable.cs:86 y :133) porque
        // en Skia el bloque se dibuja sobre un Microsoft.UI.Xaml.Controls.TextBlock -- Uno tiene
        // RichTextBlock marcado [NotImplemented] a nivel de clase para __SKIA__ -- y TextBlock no
        // tiene GetPositionFromPoint ni TextPointer: la unica superficie de Uno que sabe
        // convertir un punto en un indice es el texto EDITABLE (TextBox). Sin ese mapa el gestor
        // no puede anclar la seleccion, y no es un cableado que falte sino una medida de texto
        // que hay que escribir.
        //
        // Pintar, en cambio, si se podria: Select(start, end) no tiene ninguna guarda y usa
        // TextHighlighters, que TextBlock tambien tiene. O sea que lo que falta es el gesto, no
        // el resalte.
        //
        // CONSECUENCIA SOBRE Ctrl+C, y es la parte que hay que entender bien: ChatView reparte
        // `if (selector.HasSelection) CopySelectionToClipboard(); else CopyMessage(...)`. Con
        // HasSelection en false constante, Ctrl+C copia SIEMPRE el mensaje entero. En Windows
        // copiaria lo seleccionado -- pero aqui no hay forma de seleccionar nada dentro de una
        // burbuja, asi que copiar el mensaje entero es la respuesta correcta, no una respuesta a
        // medias: no hay ninguna seleccion en pantalla que el portapapeles este ignorando.
        ...
        public FormattedText GetSelectedText() => null;
        ...
        public void UpdateSelectionEnabled(bool value, bool animate)
        {
            if (_message is MessageViewModel message && _templateApplied)
            {
                ...
                if (animate)
                {
                    ...
                    {
#if LINUX
                        // Uno's Compositor.RegisterAnimation indexes its dictionary by the animation
                        // OBJECT, so starting the very instance the icon is already running (a few
                        // lines up, "Translation.X") on a second visual throws "An item with the
                        // same key has already been added" — and from a selection-mode toggle that
                        // escapes into the pointer dispatch and kills the dispatcher. One instance
                        // per visual, same keyframes; the Windows path is left byte for byte.
                        var slide = BootStrapper.Current.Compositor.CreateScalarKeyFrameAnimation();
                        slide.InsertKeyFrame(0, value ? 0 : 36);
                        slide.InsertKeyFrame(1, value ? 36 : 0);

                        presenter.StartAnimation("Offset.X", slide);
#else
        ...
        public partial class MessageSelectorAutomationPeer : ToggleButtonAutomationPeer, ISelectionItemProvider
        {
            ...
            protected override string GetNameCore()
            {
                ...
                else if (_owner.ContentRoot() is MessageBubble child)
                {
