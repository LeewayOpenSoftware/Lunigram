...
namespace Telegram.Controls
{
    ...
    public partial class ProfilePicture : ControlEx
    {
...
#if LINUX
        // Same string as the TelegramThemeFontFamily key of App.xaml: Uno truncates a
        // FontFamily.Source at the first '#', so what is resolved is the packaged file itself.
        private static readonly FontFamily GlyphFontFamily = new("ms-appx:///Assets/Fonts/Telegram.ttf#Telegram");

        // Whatever the template gave Initials, kept so switching back is exactly that and this
        // does not have to know which resource key the template named.
        private FontFamily InitialsFontFamily;
#endif
        ...
        protected override void OnApplyTemplate()
        {
...
#if LINUX
            InitialsFontFamily = Initials?.FontFamily;
#endif
            ...
            InvalidateShape();
#if LINUX
            // The Source can be assigned before the template exists (SettingsPage assigns it from
            // UpdateUser, driven by the view model rather than by the page's load cycle), and every
            // paint path is guarded on LayoutRoot, so that assignment silently painted nothing.
            // The presenter was created and attached by that first pass; it simply had no
            // LayoutRoot to paint into, and Invalidate returned early without saying so.
            //
            // Calling Load() again here is NOT enough, and being a no-op is precisely the bug the
            // earlier attempt shipped. Load() reaches
            //     if (_presenter == null || _presenter.Presentation != presentation)
            // which asks "do I need a DIFFERENT presenter", not "am I attached to this one".
            // ProfilePicturePresentation is a positional record, so an unchanged Source and Size
            // compare EQUAL by value: the gate is false, the body is skipped, and nothing ever
            // paints into the LayoutRoot we just built. Blank avatar, no initials, no exception,
            // correctly sized -- which is why it survived every build and every screen pass.
            //
            // So re-attach explicitly instead of asking a gate whose whole job is to say "already
            // loaded". Presenter.Load re-registers into a HashSet (idempotent) and immediately
            // replays the resolved source through Invalidate, which now has a LayoutRoot to paint
            // into; its own Load(State.Download) is a no-op while the file id is unchanged.
            // See u-settings-own-avatar-missing.
            if (_presenter != null)
            {
        ...
        private void Load()
        {
            var source = Source;
#if LINUX
            // IsConnected comes from ControlEx._loaded, which is only set when the Loaded event
            // fires AND GetParent() happens to be non-null at that instant. For a control that is
            // the Content of a ContentPresenter inside a ControlTemplate -- which is what
            // SettingsPage's avatar is, sitting in a HyperlinkButton -- Parent and
            // VisualTreeHelper.GetParent can both still answer null there (the file's own comment
            // says as much for DataTemplate roots), so _loaded never flips and Load() vetoes
            // itself forever. Nothing re-runs it: OnApplyTemplate only calls InvalidateShape.
            // The symptom is a blank avatar with no initials either, because Load() is the single
            // path to BOTH. Widened rather than replaced: IsLoaded is the framework's own answer to
            // "am I in the tree", so this can only ever let a genuine load through, never suppress
            // one. See u-settings-own-avatar-missing.
            if (source != null && (IsConnected || IsLoaded))
#else
...
#if LINUX
                    // The template draws Initials in EmojiThemeFontFamilyWithRounded, which on
                    // Linux is one plain family (Uno reads FontFamily.Source as a single name, see
                    // Theme.UpdateEmojiSet), so a Telegram.ttf glyph gets here through
                    // LinuxFontFallback - and Uno asks FeatureConfiguration.Font.SymbolsFont first.
                    // 335 of Telegram.ttf's codepoints are also in that Fluent icon font, U+EA61
                    // (the Saved Messages avatar) among them, so the wrong icon was drawn and
                    // nothing said so. Naming the file as the run's own family skips the fallback.
                    Initials.FontFamily = _glyph
                        ? GlyphFontFamily
                        : InitialsFontFamily;
#endif
                }
...
#if LINUX
                polygon.SetPath(GetTail(Size / 2f)); // Path is typed on Uno's CompositionPath, see Telegram.Linux/Graphics/CompositionPath.cs
#else
