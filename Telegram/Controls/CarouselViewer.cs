...
namespace Telegram.Controls
{
    ...
    public partial class CarouselViewer : Grid
    {
        ...
        public bool IsManipulationEnabled
        {
            ...
            set => _isManipulationEnabled = value;
#else
...
#if LINUX
        // The settle animation is not the tracker's inertia, so the 100 ms window is not enough on
        // its own: while it runs, the host must keep refusing a second pass.
        public bool IsScrolling => _settling || Logger.TickCount - _scrolling < 100;
#else
        ...
        {
#if LINUX
            ChangeViewLinux(direction, disableAnimation || !PowerSavingPolicy.AreSmoothTransitionsEnabled);
#else
...
#if LINUX
        // The Windows half re-anchors the tracker and lets the expression animations recompute every
        // offset; here the same call just rewrites the three translations for the current progress.
