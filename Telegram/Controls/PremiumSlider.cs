...
namespace Telegram.Controls
{
    public partial class PremiumSlider : Slider
    {
        ...
        protected override void OnValueChanged(double oldValue, double newValue)
        {
            ...
            {
#if LINUX
                // Uno's CompositionTarget.Rendering is a frame counter, not a frame clock: its rate
                // follows whatever happens to be invalidating the window, so an animation that
                // draws from it is its own metronome. See
                // Telegram.Linux/Xaml/CompositionRenderingClock.cs.
                CompositionRenderingClock.Rendering += _rendering = new EventHandler<object>(OnRendering);
#else
