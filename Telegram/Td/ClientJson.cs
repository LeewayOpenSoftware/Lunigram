...
namespace Telegram.Td.Api
{
    ...
    public partial class ClientJson
    {
        ...
        // Writer and buffer are only ever created together (see below), but as two independent
        // [ThreadStatic] fields the compiler can't see that invariant across the if/else that sets
        // them, hence the CS8604 this used to carry on the else branch. Bundled into one holder so
        // "created" is a single non-null check instead of two fields that have to stay in sync.
        private sealed class JsonScratch
        {
            public readonly Utf8JsonWriter Writer;
            public readonly FreeBufferWriter Buffer = new();

            public JsonScratch(IBufferWriter<byte> initial)
            {
                Writer = new Utf8JsonWriter(initial, _options);
            }
        }

        [ThreadStatic]
        private static JsonScratch? _scratch;
        ...
        public static ReadOnlySpan<byte> ToJson(ArrayPoolBufferWriter buffer, Function obj)
        {
            ...
            if (scratch == null)
            {
                scratch = _scratch = new JsonScratch(buffer);
            }
            else
            {
                scratch.Writer.Reset(buffer);
            }
        ...
        // The generated DoFromJson/FromJson_* chain declares ClientResultHandler non-nullable
        // because File/UpdateFile are the only two types in the whole schema that ever dereference
        // it (see ApiEmitter.IsHandledByClient) — everything else threads it through unused. Real
        // callers legitimately pass handler: null when they know the response can't be one of
        // those two (Client.cs's synchronous td_execute path, RichHtml's instant-view parsing).
        // Rather than widen nullability through every generated signature for two call sites, give
        // the boundary a sentinel that fails loudly and specifically if that invariant is ever
        // wrong, instead of a bare NullReferenceException.
        private sealed class NullResultHandler : ClientResultHandler
        {
            public static readonly NullResultHandler Instance = new();

            public void OnResult(Object result)
            {
                throw new InvalidOperationException("NullResultHandler.OnResult should never be reached from the FromJson parse path.");
            }

#if TD_READER_PARSER
            public UpdateFile ParseUpdateFile(ref Utf8JsonReader reader)
            {
                throw new InvalidOperationException("A File/UpdateFile response reached FromJson without a ClientResultHandler; only Client's update dispatch may parse those.");
            }

            public File ParseFile(ref Utf8JsonReader reader)
            {
                throw new InvalidOperationException("A File/UpdateFile response reached FromJson without a ClientResultHandler; only Client's update dispatch may parse those.");
            }
#endif
        ...
        public static Object FromJson(ReadOnlySpan<byte> jsonData, ClientResultHandler? handler = null)
        {
            ...
            if (reader.TokenType == JsonTokenType.StartObject)
            {
                ...
                if (reader.TokenType == JsonTokenType.PropertyName && reader.ValueTextEquals("@type"u8))
                {
                    ...
                    reader.Read();
                    return DoFromJson(ref reader, handler ?? NullResultHandler.Instance, hash);
                }
            }
