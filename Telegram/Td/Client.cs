...
namespace Telegram.Td
{
    ...
    public partial class Client
    {
        ...
        private static void LogMessageCallbackWrapper(int verbosity_level, IntPtr message)
        {
            ...
            if (callback != null)
            {
                // TDLib's own log callback always hands a real C string; PtrToStringUTF8 is only
                // nullable because it also has to answer for IntPtr.Zero, which this call site
                // never sees.
                callback(verbosity_level, Marshal.PtrToStringUTF8(message) ?? string.Empty);
            }
        }
    }
}
